...
- [ ] **Review stop-unsupported-completion.sh precision after 14 days** — After 2026-04-25: inspect `~/.claude/unsupported-completion-shadow.jsonl` for agent-infra sessions. Compute `would_fire=true` count and sample 30 fires for manual precision labeling. If ≥60% precision and trigger rate <10% of eligible sessions, propose promotion to advisory mode AND propose global `~/.claude/settings.json` registration (currently meta-only per Principle 4). If <60% precision, narrow predicate (maybe require ≥3 success hits, add more evidence-language patterns) and restart shadow period. (Source: bias-tooling plan 2026-04-11, skills@c0b3f71, FM25 in agent-failure-modes.md)
