...
- [ ] **Spec-and-verify workflow experiment** — Design a task-contract format (requirements, acceptance criteria, verification commands). Run matched experiments: same task done interactively vs spec-and-verify. Measure turns, cost, quality, correction rate. Start with genomics (highest total cost) and meta maintenance. Prerequisite: 1 week of Phase 0 baseline telemetry from token reduction hooks. GPT-5.4 estimates 10% turn reduction on top-decile = ~$610/month savings. Write divergent-options.md before implementing (Constitution P6). (Source: token reduction plan 2026-04-07, both review models promoted this)
- [ ] **Session checkpoint crossover experiment** — Multi-day sessions dominate cost outliers (arc-agi: $66/session avg). Splitting may help or hurt depending on cache-reset penalty. Run randomized crossover on 10 long sessions: "continue" vs "checkpoint at $30 + restart." Measure total cost and quality per arm. Do NOT build auto-checkpointing infra until experiment shows it's cheaper. (Source: token reduction plan 2026-04-07, GPT-5.4 #3/#23)
- [ ] **Review Phase 0 telemetry after 1 week** — After 2026-04-14: run `scripts/token-baseline.py --days 7`, check `~/.claude/read-discipline-shadow.jsonl` for full-file read patterns, evaluate whether read-discipline hook should promote from advisory to warning. Check if terse-bash suggestions are being followed via hook-triggers.jsonl. (Source: token reduction plan Phase 2 gate)
- [ ] **Governance-drift cross-model verifier** — Advisory hook on Write/Edit to `CLAUDE.md`, `GOALS.md`, `.claude/rules/*.md`, constitution sections. Dispatches diff to Gemini Flash (free CLI) with "find goal drift or sycophantic alignment with the editing agent." Both cross-model reviewers on 2026-04-11 bias-tooling plan endorsed this as the targeted sycophancy-verifier cousin. Deferred because `improvement-log.md` grep for governance-file-drift incidents returned zero hits — fails Rule 1 (no incident history = hypothetical). **Build trigger:** ≥2 sessions of confirmed governance-drift recurrence logged to improvement-log by session-analyst. (Source: `.model-review/2026-04-11-bias-tooling-plan-4ba85d/` findings 12, 19)
- [ ] **Review stop-unsupported-completion.sh precision after 14 days** — After 2026-04-25: inspect `~/.claude/unsupported-completion-shadow.jsonl` for agent-infra sessions AND `~/.claude/unsupported-completion-errors.jsonl` for any silent hook failures. Also sample ~20 sessions where the hook SKIPPED (no shadow entry) to estimate recall — how many contained unsupported-completion patterns the lexical predicate missed. Promotion criteria: (a) ≥60% precision on sampled fires, (b) ≥50% recall on sampled skipped sessions, (c) trigger rate <15% of eligible sessions (gives headroom beyond 10% since lowering `success_hits >= 1` will raise base rate), (d) zero entries in the error log. If all four: propose promotion to advisory mode AND global `~/.claude/settings.json` registration (currently meta-only per Constitution Principle 4 on reversibility + blast radius). If any criterion fails: narrow the predicate (e.g., require specific evidence markers, add transcript tool-output inspection), restart shadow period. Recall floor added per plan-close review finding 15. (Source: bias-tooling plan 2026-04-11, skills@e3b89c0, FM25 in agent-failure-modes.md)
- [ ] **agent-belt (jfrog) dep evaluation** — Eval harness that tests the *actual CLI binary* (multi-judge consensus, pass^k variance, paraphrase families, worktree/Docker isolation). Closest external match to ~/Projects/evals/, which only tests at the API layer — the harness+model composite is untested today. Evaluate as dependency first (per evals-macvb-home: survey evals/ before building); extract the test-the-real-binary pattern if it fails due diligence. (Source: trending-scout 2026-06-11, finding 4)
- [ ] **ralph-loop Stop-hook block-loop for bounded "grind to done" tasks** — Stop hook returns `{decision:"block", reason:<same prompt>}` to feed the prompt back in-session until the agent emits a literal `<promise>TRUE</promise>` tag or hits `max_iterations`; state in `.claude/ralph-loop.local.md` frontmatter (iteration/max/completion_promise/session_id). The win over our `/loop`: **zero scheduling quota** (no ScheduleWakeup/CronCreate — the wakeup-cadence rule's ~15/day account ceiling doesn't apply to an in-session Stop-block) and a clean completion-predicate exit, vs `/loop`'s interval/self-pacing. Distinct niche from `/improve maintain` (RSI conductor) and `/loop` (recurring interval). NOT strictly better — fills a gap neither covers: "hammer ONE task until a boolean is true, quota-free." Risks to design against: the "do not lie to exit" honesty dependency ([[fable-5-leverage]] honesty regression → keep verify-hooks in the loop) and runaway cost (cap max_iterations, surface spend). **Build trigger:** ≥2 sessions where a bounded grind-to-done task burned ScheduleWakeup quota or stalled on `/loop` self-pacing. Source: anthropics/claude-plugins-official `plugins/ralph-loop/hooks/stop-hook.sh`. (Source: marketplace-plugin source audit 2026-06-14)
- [ ] **Named "fail-loud / silent-failure" review lens for /critique or /code-review** — pr-review-toolkit ships `silent-failure-hunter` (flags empty catch blocks, broad excepts, unjustified fallbacks, swallowed errors, return-null-on-error-without-logging, retry-exhaustion-without-surfacing) and `type-design-analyzer` (make illegal states unrepresentable; validate invariants at construction) as dedicated parallel review agents. The *idea* maps onto principles we already hold — epistemic P8 (silent-proxy-as-truth / fail-loud), `probe-primitive-first` "fail loud on dead data plane", `invariants.md`. Our `/critique` and `/code-review` are monolithic — no dedicated silent-failure pass. Their code is web-app-flavored (Sentry/errorIds.ts/Statsig) → port the *lens*, not the code, to Python/research-infra (swallowed subprocess errors, bare `except:`, silent fallback to a proxy source per P8). **Build trigger:** a confirmed silent-failure/swallowed-error bug `/code-review` missed in ≥2 sessions. Source: pr-review-toolkit/agents/{silent-failure-hunter,type-design-analyzer}.md. (Source: marketplace-plugin source audit 2026-06-14)
- [ ] **Read 2606.06324 + 2606.08348 before session-learning Phases 1-4 resume** — harness-flaws-as-root-cause (diagnostic taxonomy for the hook/forensics loop) and skill-revision-as-posterior-updating (one observed failure ≠ overwrite the skill — weight it). Both directly shape the reflective skill-rewriting phase. Frontier-tested status unverified. (Source: trending-scout 2026-06-11, finding 3)
- [x] **SHIPPED 2026-06-16 (skills@2b6da1b + d54f540, approved; `just critique` triage restored agent-infra@453ef53). Mode-gate `_scan_dead_refs` (block on `--mode close` only) + provenance-gate the `dispatch.json` auto-load + GOALS-charter-OFF-by-default for adversarial critique.** Test `critique/scripts/test_critique_mode_aware.py` (9, fail→pass). NOTE: gating dead-refs on `layers["diff"]["run"]` false-blocked on a dirty tree — corrected to explicit-mode-only via live integration test. Original proposal: — Two coupled bugs in `~/Projects/skills/critique/scripts/` that abort cross-model critique of **design docs** (recurred 2 sessions: 2026-06-15 + 2026-06-16). (1) `review_gate.py:_scan_dead_refs` flags any packet path not resolving in-repo as a hard BLOCKER — correct for `close` mode (diff coherence) but wrong for `model` mode, where ADRs/plans legitimately name cross-repo files (`graph_schema.sql`), tools by basename (`gov.py`, `model-review.py`), and skill-relative paths (`SKILL.md`, `lenses/…`). Fix: gate the dead-ref blocker to `--mode close` only; in `model` mode downgrade to a warning (the `just gather` `missing:` field already surfaces unresolved refs as *information*, making the blocker redundant-and-wrong there). (2) `model-review.py:2909-2911` auto-loads `.model-review/dispatch.json` whenever present, with no staleness check — a manifest from any prior triage run (this session or a peer) silently poisons the next dispatch with its blockers (`:2952` hard-returns 1). Fix: staleness-gate the auto-load (skip if the manifest's `packet_path`/`review_hash` ≠ the current packet), or require an explicit opt-in. **Recipe-local mitigation already shipped** in agent-infra `just critique` (drops triage; `rm -f .model-review/dispatch.json` before dispatch) — but the proper fix belongs in the shared skill so genomics `plan-close-review` and every other consumer benefit. Affects 3+ projects → human sign-off before edit. (Source: just-cli-dispatch-unification execution, 2026-06-16)
