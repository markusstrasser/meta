...
- [ ] **Codex 0.141 PostToolUse code-mode gating — re-verify `codex_hook_shim.py`** — "blocking PostToolUse hooks now correctly reject code-mode tool calls" (previously NOT gated). Our Codex parity enforcement assumed code-mode calls were already gated. Hands-on check that our blocking PostToolUse guards fire on Codex code-mode calls post-0.141. Source: research/trending-scout-2026-06-19.md, decisions/2026-06-02-codex-cli-project-parity.md.
- [ ] **Evaluate `Tool(param:value)` permission rules as a model-routing/cost gate** — CC 2.1.178 added `Agent(model:opus)`-style param-matching permission rules. Probe whether a declarative `Agent(model:…)` settings rule can replace/augment the PreToolUse dispatch turn-budget hook for cost routing. Single-variable settings commit if it works. Source: research/trending-scout-2026-06-19.md.
- [ ] **Pin `mcp>=1.27,<2` in MCP server deps** — mcp-py 1.28 is the last v1.x before v2 beta (Tasks API removed from spec, WebSocket deprecated). Avoid a silent v2 auto-jump breaking research-mcp / agent_infra_mcp. Source: research/trending-scout-2026-06-19.md.
- [ ] **Confirm CC 2.1.183 native destructive-git block doesn't double-prompt with our guards** — vendor now blocks destructive git in auto mode; verify no conflict with commit-time guard / `git add -A` block. Keep our guards (mode-conditional vendor coverage). Source: research/trending-scout-2026-06-19.md.
- [obs] **WATCH (memo-only, no action):** Cursor CLI `/review` STILL not shipped (risky-diff-review SHADOW gets no vendor-supersession signal this window); NousResearch hermes-agent-self-evolution = closest external RSI/skill-opt analog (extract-pattern, do NOT adopt DSPy); Human-on-the-Bridge eval paper (2606.16871) multi-juror+matched-pair design. research/trending-scout-2026-06-19.md.
