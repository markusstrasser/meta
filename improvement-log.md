...
- [ ] **PPV/reflect-eval counts STALE firings against the current predicate (windowing bug)** — the `new-script-without-test` excludes (`_probe.py`/`.sql`/`.sh`) were added `2f6b8e8`; all 9 firings predate it (2026-06-04→06-12), so they're pre-fix noise the PPV denominator can never clear → no probe can ever reach 60%. Fix: window firings to `ts > predicate_last_changed` (needs a per-rule version/mtime stamp). Blocks the omission-labeling/graduation task. Found by config-check, not labeling (probe-primitive-first). Evidence: `~/.claude/reflect-capture.jsonl` + `git log config/reflect-omission-rules.json`.
