...
- [ ] **Externalize genomics Modal pipeline state** — 1490 main-thread status-poll calls/21d (83% in main); status MCP tools at 100% error (424 calls; PRIORITIES.md `broken-cli:corpus missing modal`). Fix = `launchd`/`just watch-pipeline` polls Modal → `.genomics/state.json` (mtime staleness guard); status tools READ the cache. **Gate: probe the 100%-fail tools FIRST** (MCP-subprocess env isolation vs status-encoding artifact — probe-primitive-first), then build. Cross-model convergent (Gemini+GPT): do NOT build a watcher subagent (P1 agent-theater; 60–90K tok vs free shell poll). Evidence: `research/2026-06-14-subagent-offload-empirics-and-verdict.md`. Deferred per VOI — build behind the probe gate when genomics is next active.
