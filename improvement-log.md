...
- [ ] **llmx usage surfacing** — `llmx` has `info`/`batch` but NO per-model cost/spend/context-window stats command (operator ask 2026-06-16). `/research` whether llmx already logs usage internally (just unsurfaced) before building a `llmx usage`/`stats` view. Repo: `~/Projects/llmx`.
