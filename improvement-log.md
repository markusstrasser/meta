...
- **[x] bare-python3 env crashes (agent-invoked):** `missing-module:duckdb`×46, `pipeline_stages`×40, `edgar`×32 / 21d — all interactive agent bare-`python3`, not cron. **Fix:** PreToolUse bash nudge → `uv run` when project has `pyproject.toml`/`.venv`; doctor already surfaces. **Shipped 2026-06-21:** `pretool-uv-python-guard.py` auto-rewrite when cwd has pyproject.toml/uv.lock.

### [2026-06-21] Leverage residue — WIN 3+4 (verified strictly better)
- **[x] predict-then-falsify ablation closure:** `maintain_tick.run_subtract(..., ablate=True)` now calls `resolve_predictions_for_ablation()` when ablation is conclusive (`ablated=True` → refuted, `False` → confirmed); conservative gov_id/artifact match on prediction change text. Test: `test_resolve_predictions_for_ablation_matches_gov_id`.
- **[x] agentlogs `status_update` payload reclaim:** `agentlogs compact` NULLs write-only `status_update` payloads + VACUUM; ingest already drops via `trim_payload`. Measured dry-run: 67,724 rows / ~190MB payload on 4.7GB DB. **Code shipped; apply pending indexer lock:** `uv run agentlogs compact --yes`.

### [2026-06-21] Observe v3 — eight net-new promotable (14d, genomics·phenome·intel·corrections)
- **Source:** `artifacts/observe/2026-06-21-v3/` — expanded scope beyond v2 agent-infra paper batch; Cursor subagents, no Flash.
- **[ ] `run_reconcile` receipt-blind dead_letter:** SUCCESS + no live Modal app → `dead_letter` without reading volume receipt (`7ffc759b` blocked; fixed path in `1a19b57e` / `03dd0812e`). Aligns audit **PASS4-05**. **Fix:** audit all terminal classifiers for receipt-blind paths + regression test. **Root cause:** system-design. **Stream:** genomics orchestrator.
- **[ ] `attest_sample inspect` omits `attempt_dir` in human output:** `cmd_inspect` prints `stage: denom=… files=…` only; attestor prompt contract needs `attempt_dir` to bind verdicts (6+ Codex attest sessions). **Fix:** print or `--json` emit `attempt_dir` per stage. **Root cause:** skill-execution. **Stream:** genomics script.
- **[ ] Control-plane CUT vs additive peer drift:** coordinator executes Bluesky CUT while peers land cert-pass-2 / Side-B half-measures (`1a19b57e` ↔ `7ffc759b` Modal→local phase mismatch). **Fix:** worktree + explicit peer freeze during control-plane cuts. **Root cause:** system-design. **Stream:** architectural (multi-agent).
- **[>] Codex loads stale `~/.codex/skills/genomics-pipeline`:** 6/8 Codex genomics sessions vs repo `.claude/skills/`. **Partial (2026-06-28):** `codex-parity --repo genomics` + model-guide dispatch doc + genomics CLAUDE.md repo-first skills. Residual: enforce in codex exec templates / lint stale global copy age.
...
- **[obs] Corrections cluster — first-party docs / search-before-propose:** 26 KPI rediscovery / 14d; routes to in-flight prior-context v2 `existing_infra` slice — not a new build.

- [obs] **Ran the hermes SkillModule loop on `arc-agi llm_codegen_solver._build_prompt` (prose=param) vs ARC exact-match (true ground-truth verifier, no Goodhart), 6 ARC-1 tasks, gemini-flash.** Final: baseline TRAIN tm=1.00/HOLDOUT tm=0.67; **v2 TRAIN tm=1.00 (tied baseline) but HOLDOUT tm=0.33 — collapsed**; v1 TRAIN tm=0.67 (regressed). Baseline (hand-written) won. **The sharp result: v2 was indistinguishable from baseline on the SELECTION set but halved on holdout — a live demo of the overfit trap; a train-only tiebreak would have shipped a holdout regression.** Confirms on ARC what extraction showed: prompt-body auto-opt (a) needs the constraint-gate (b) only pays where a prompt-addressable gap exists — a strong hand-written baseline leaves little, and naive rewrites overfit or regress. ARC is the IDEAL verifier setting (exact, cheap, ground-truth) yet auto-opt still lost → the binding constraint isn't the verifier, it's headroom. Reinforces [[skillopt-vs-autobrowse-veto]]: clear verifier is necessary, NOT sufficient. Files: arc-agi experiments/skill_optimize_codegen_prompt.py (acb576a), /tmp/codegen_skillopt.log.

### [2026-06-19] Skill-optimize demo — hermes SkillModule pattern, run on a real verifier
- [obs] **Ran the hermes propose→eval→select loop on a duplicate of `phenome/prompts/extract_claims.md` vs a ground-truth metric — baseline WON, both LLM rewrites regressed.** Used the parked DSPy scaffold in evals (`generic_metric.score_extraction` + 4 frozen dev golds; live prompt read-only), gemini-3-flash. Mean combined: baseline 0.597 > v1-recall 0.528 > v2-precision 0.378. **Durable lessons (the point, not the score):** (1) **constraint-gate is mandatory** — v2's naive body-rewrite broke the JSON output contract → parse-fail on 2/4 docs; a re-validate-output gate (which hermes has, my minimal loop lacked) would auto-reject it. (2) **single scalar metric hid a regression** — v1 lifted recall but collapsed cetp trap_avoid 1.00→0.50 (emitted a forbidden meta-claim); only the per-component breakdown caught it → optimize on a gated/vector objective, never one scalar. (3) **doc-type headroom map:** clean gene pages at ceiling (~0.98), paper-type far below (estbb recall 0.10–0.30, johansson parse-fails) — confirms `extract_claims` paper-extraction is the real gap, but the fix is schema/prompt work, NOT auto-opt (which made papers worse). Empirically closes the "screening prediction never run" gap in evals/extraction_bakeoff/DSPY_feasibility.md §4 — PARK upheld. Cost ~14 flash calls / ~843s / <$0.10. Reinforces [[skillopt-vs-autobrowse-veto]]: skill-opt pays only behind a clear verifier AND a constraint-gate. Full: evals/extraction_bakeoff/DSPY_feasibility.md (2026-06-19 rev), skill_optimize_demo.py.

### [2026-06-19] Steering vectors — incremental steer-mine (29 sessions, 495 signals)
- **Source:** `just steer-mine` incremental pass → `artifacts/observe/2026-06-19-steer-signals.jsonl`; clustered with `probe-2026-06-17.jsonl` (1,314 total signals). Synthesis: `research/2026-06-19-steering-vectors.md`.
- **[obs] Verify-then-act is dual-register, not contradictory.** 255 verify-first steers vs 64 act-now — user verifies *claims* but expects *execution* on already-authorized scope. Confirms verifier-conditioned GOALS framing; don't collapse into "always ask" or "always act."
- **[obs] Over-ask persists post-autonomy pushes.** 64 corpus hits ("say the word", end-of-turn permission loops); aligns with blindspot `over_caution` (59/7d). Not a missing instruction — architectural Stop/UserPrompt boundary.
- **[obs] Prior-context hook residuals are `existing_infra`, not git-log blindness.** Triage 20/25 flags = schema/view/hook already exists; steer-mine prior_context theme (89 hits) is mostly "we fixed codex hooks yesterday" class — search-before-propose, not more git-log injection.
- **[ ] continuation-misread hook:** agent replies "No response requested" to "Continue from where you left off" — 4+ sessions (`emb pairs` cluster), identified 2026-06-17 embed-once memo, still firing Jun 19. **Fix:** UserPromptSubmit or Stop — continuation directive must not resolve to no-op. Checkable predicate.
- **[ ] Codex global hooks fleet shim:** parity-sync fixed project mirrors but 63 hand-maintained `~/.codex/hooks.json` hooks remain unshimmed (phenome regression steers). **Fix:** extend `codex_parity_sync.py` to global fleet or document explicit boundary + audit script.
- **[ ] HUMAN.md template-aware open counter:** `agi3-status` false-matched `[open]` in format-template example. **Fix:** anchor grep on real escalation headers only (hutter).
- **[-] Re-build prior-context v2 / agentlogs auto-prune:** both shipped (2026-06-17); drift finding stale — `reclaim-rotate` pruned 754MB today 02:10.

### [2026-06-19] `if:`-gate the git-only PreToolUse[Bash] hooks (elegance/perf, user-directed)
- [x] **Gated 5 git-only Bash hooks behind `if: "Bash(git*)"` in `~/.claude/settings.json`** — `git-noext-inject`, `git-add-all-guard`, `no-background-commit`, `multiagent-commit-guard`, `destructive-git-ref`. Before: 19 unconditioned `PreToolUse[Bash]` hooks fired+self-filtered on EVERY Bash call (~2.9s sequential wall-time measured on a no-op `echo`, /tmp/test_git_hooks.py; the 5 git hooks = ~1.2s of it). Native `if:` (CC 2.1.89+ compound-split prefix match) prevents the SPAWN on non-git calls (the common case). **Tested+simulated** (user-requested): ran all 5 scripts on git/non-git/compound/`git -C`/background payloads + simulated the gate — **zero false-negatives** (every command a hook ACTS on, the gate PASSES, incl. `cd x && git reset --hard HEAD` and `git -C /p commit`); each hook still self-filters internally so the gate only saves the spawn. Mechanism proven live: `commit-check` (already gated `Bash(git commit*)`) = 1768 fires across 44 projects (`just hook-telemetry`). JSON validated (hooks not disabled). Left modal/python/duckdb guards UNGATED — substring triggers (`uv run modal`) would slip a prefix `if:` = silent safety-guard false-negative. NOTE: change is LIVE in the file but UNCOMMITTED — `~/.claude/settings.json` also carries the operator's own pending config toggles (effortLevel/verbose/theme/MessageDisplay); operator commits. Backup: /tmp/settings.json.bak-*. Reversible.
- [obs] **Surfaced (not mine to fix here): `destructive-git-ref` only warns on reset/checkout WITH an explicit unstable ref** (HEAD/branch-tip/origin) — bare `git reset --hard` and `git checkout -- <path>` do NOT warn (by design per its header, but the `-- <path>` case still destroys uncommitted file changes; coverage may be too narrow). Pre-existing; flagged for a separate look.

### [2026-06-19] Hermes deep-dive (local clone) — forensic confirmation, no second steal
- [obs] **GEPA never executes in the clone — the headline is triply unwired.** Against pinned `dspy==3.2.1`, `dspy.GEPA(metric=..., max_steps=...)` (evolve_skill.py:156-159) raises TypeError (`max_steps` not a GEPA param) → bare `except` at :166 silently falls back to MIPROv2 EVERY run; `reflection_lm` never passed (required, defaults None); metric is a bare float (fitness.py:136, no feedback). 145 passing tests cover importers/constraints/parsers only — never the optimizer path. Per-category vs ours: GEPA reflection prompt WORSE (dead path), Pareto/selection WORSE (`population_size:5` has zero readers), constraints.py contract SAME (clean `ConstraintResult` shape but `run_test_suite` zero callers), skill_module.py BETTER-narrow (the one steal, parser bug: `split("---",2)` breaks on `---`-using bodies — use a real frontmatter parser), dataset/importers WORSE (LLM-imagined evals vs our agentlogs ground-truth). Three micro-primitives noted: anchored `SECRET_PATTERNS` scrub regex (real gap for external-corpus ingestion), brace-balanced JSON extractor, LLM error-rate reporting. **Verdict: NO second capability steal** — `SkillModule` parameterization stands as the only one, adopt only behind a ground-truth verifier per [[skillopt-vs-autobrowse-veto]]. Full: research/2026-06-19-hermes-deep-dive-local.md.

### [2026-06-19] Trending scout — vendor sweep (window Jun 15→19)
- [x] **Codex 0.141 PostToolUse code-mode gating — VERIFIED, no shim change needed** — 0.141 "blocking PostToolUse hooks now correctly reject code-mode tool calls" is strictly ADDITIVE coverage. `codex_hook_shim.py` already speaks the Codex blocking contract (rc==2 → reason lifted to stderr, lines 285-286; PostToolUse in JSON_STDOUT_EVENTS). Verified: tests 8/8 pass (exit-2-block path covered), live `~/.codex/log/hook_shim_invocations.jsonl` shows 52 PostToolUse fires + 3 exit-2 blocks, latest 2026-06-19T11:06 — hooks fire live under current codex. Code-mode calls now gated too = free coverage gain. Source: research/trending-scout-2026-06-19.md, decisions/2026-06-02-codex-cli-project-parity.md.
- [-] **Pin `mcp>=1.27,<2` — REJECTED (over-cautious, premise falsified on probe)** — `mcp` is a TRANSITIVE dep (`Required-by: claude-agent-sdk`, installed 1.27.0); our MCP servers depend on `fastmcp>=3.4.2,<4.0` which OWNS the mcp ceiling. mcp 2.x is not on PyPI yet (1.28 = "last v1.x before v2 beta") so a resolver cannot jump to it, and a manual `mcp<2` cap would override fastmcp's own compatibility declaration (proxy-as-truth anti-pattern). Real trigger: revisit when mcp 2.0 actually ships AND fastmcp 3.x permits it. Source: research/trending-scout-2026-06-19.md.
- [x] **CC 2.1.183 native destructive-git block — no conflict, keep our guards** — native block is PreToolUse-equivalent and auto-mode-only; our `pretool-destructive-git-ref.sh` / `git-add-all-guard` fire in ALL modes + survive Codex. Worst case in auto mode = redundant block (not a double-PROMPT — blocks don't prompt), harmless. Vendor coverage is a subset of ours; keep ours unchanged. Source: research/trending-scout-2026-06-19.md.
- [ ] **Evaluate `Tool(param:value)` permission rules as a model-routing/cost gate** — CC 2.1.178 added `Agent(model:opus)`-style param-matching permission rules. ADDITIVE (we have no subagent model-gate today) — introducing one is a policy choice, not a mechanical fix, so left open as a genuine evaluate: would a declarative `Agent(model:…)` rule beat advisory model-guide prose for cheap-lane cost routing? Source: research/trending-scout-2026-06-19.md. **Revalidated 2026-07-04 (drain):** still open and now MORE actionable — zero `Agent(` rules deployed anywhere, and CC 2.1.186 made enforcement live; distinct from the (done) L4008 audit.
- [obs] **WATCH (memo-only, no action):** Cursor CLI `/review` STILL not shipped (risky-diff-review SHADOW gets no vendor-supersession signal this window); Human-on-the-Bridge eval paper (2606.16871) multi-juror+matched-pair design. research/trending-scout-2026-06-19.md.
- [obs] **RECONCILIATION — hermes-agent-self-evolution extracted (no new build).** Deep-read (research/2026-06-19-hermes-self-evolution-pattern.md): the one transferable idea is *skill-body-as-optimizable-parameter* (wrap SKILL.md body as the GEPA-mutated param) — but it's adopt-only-behind-a-clear-verifier, which our [[skillopt-vs-autobrowse-veto]] already pre-registers (gated on scored evals). Skip their GEPA keyword-overlap metric (Goodhart), skip session ingestion (agentlogs.db strictly beats their flat prompt-only importer), and their PR write-back is UNWIRED (`create_pr`/`run_test_suite` zero callers; loop ends by printing a diff). Our improvement-log promotion path is more mature. Net: confirms our loop's shape, triggers no build. Source: research/2026-06-19-hermes-self-evolution-pattern.md.

### [2026-06-17] Recurring agent_miss clusters — semantic mining (embed-once)
- [obs] **35 of the top recurring "mistakes" are GATE-INDUCED friction, not discipline failures.** Semantic clustering (emb pairs over 595 agent_miss signals, ~85 operator sessions) surfaced: subagent dispatch-gate retry (18, write-stub/file-output rejected 1st try), provenance-tag Stop-hook block (17), shared-checkpoint/peer-session clobber (17), commit-without-request (12), asked-when-instructed (9), bare modal/python (6). 50% of all misses forced a steer. **Meta:** top-2 clusters = our own gates blocking the first attempt → fix gate ERGONOMICS (transform/auto-inject, not block-then-retry), same shape as the poll-hook bare-dir false-fire (skills@0e6ac79). Global candidates (gate-ergonomics first, per principle 3): subagent-gate auto-include strings, provenance auto-scaffold, peer-session auto-worktree. Evidence: research/2026-06-17-embed-once-validated-recurring-mistakes.md.

### [2026-06-16] Session-end RSI reflection — two deep proposals
- [x] **Usage telemetry — subscription blind spot closed (`llmx@7e2d993`)** — light critique (cursor repo-grounded + gpt-5.5) REPLACED the tiktoken plan: `claude -p --output-format json` returns REAL tokens + API-equivalent `total_cost_usd` even on subscription (verified live). Wired `log_usage` at the `cli_chat` chokepoint (claude path), `transport=claude-cli`, cost = API-equivalent (not spend). Subsidy now measurable for the dominant case. **Follow-ups [ ]:** codex/cursor-agent json parsing (same chokepoint); usage_report transport-aware actual-vs-equivalent split (gpt's provenance point).
- [x] **Pulse liveness contracts — scoped, NOT universal (`bf68c3a`)** — both critiques killed the "universal registry": a uniform constancy test alarms-on-green for healthy-constant metrics (doctor/test-health) and INFERRED variance would launder the AIR-1591 constant-0 as normal. Shipped: per-metric DECLARED contract (should-vary/may-rest), constant-test only for should-vary, + `supervision.correction_load`. blindspot-rate deferred (no stable producer probe).
- [>] **Producer-side field contract (the bigger lever — both reviewers, ADR-candidate)** — **first slice 2026-06-28:** `supervision_session.build_report()` emits `metric_integrity.warnings` + stderr `METRIC_INTEGRITY` when `sessions≥5` and `hooks_shown=0`; `reflect.py` quarantine attach → `lifecycle: modify`. Residual: generalize to other metric producers + ADR.
- [obs] **Dominant supervision tax this session was TIMIDITY, not error** — ~6 operator corrections, almost all "stop deferring/asking/ceremony" (over_caution: "queued?", "why ask", P12-as-ceremony), not "you're wrong." Matches blindspot RAISE_AUTONOMY=47. The objective ("declining supervision") is well-targeted; today both the detector (over_caution graduated shadow→enforce) and the metric (AIR live, pulse canary) came online to measure THIS tax. Test: does the raise_autonomy correction-rate fall over coming weeks. The loop is now instrumented on its real bottleneck.
### [2026-06-16] RSI closure loop was open — dead instrument welded the promotion valve shut
- [x] **Closure metric repointed (`supervision-kpi.py`)** — `hooks_shown`/`air` read a `progress/hook_progress/Stop` signal present 7× across all transcripts instead of `attachment/hook_success` (>3600×). Result: `hooks_shown=0`, `air=null` on 1591 sessions — the loop's one closure metric measured nothing. Fixed → 5388 shown / 286 after / mean AIR 0.032 over 14d. `agent-infra@5563996`.
- [x] **`over_caution` graduated shadow→enforce (soft nudge)** — `stop-smart-judge.sh` vector #3 ("ends by asking permission for an obvious, reversible, already-authorized action") had been catching this exact failure 8× on 2026-06-16, all silent in shadow. Operator directed it ("…is a hook"). Set `SMART_JUDGE_MODE=enforce` + added `over_caution` to enforce-vectors in `~/.claude/settings.json`. Soft nudge (Stop `decision:block` re-prompt, never destructive); reversible (remove the two env keys). **Tradeoff flagged:** claude-sub transport adds 27–38s on prefilter-flagged turns only; fast path = metered haiku-api (against subscription-default policy).
- **Structural diagnosis (the "how good is the derivation machinery" answer):** the SENSE layer works — detectors correctly fire (8 clean over_caution catches in one day). The loop fails to CLOSE because the promotion gate ("graduate a shadow detector once precision is earned") depended on a closure metric that was dead. Sense → shadow → [welded valve] → never graduates → same misses recur → human re-catches manually. Over-built front half, disconnected back half. Fix = re-point the instrument (done) so graduation can run on real precision. Evidence: this session's transcript; `~/.claude/smart-judge-shadow.jsonl` (57 verdicts, 8 over_caution).
- [x] **PPV/reflect-eval counted STALE firings against the current predicate (windowing bug)** — FIXED: added per-rule `since` (predicate-last-changed date) to `config/reflect-omission-rules.json`; `reflect_eval._since_map()` windows the PPV count to firings on/after it. Result: new-script (9, since 06-13) + genomics dispatch/stage (since 06-16) now excluded as stale; only the 11 valid intel firings (entity-write 6 + state-claim 5, predicate unchanged since 06-03) count. The report shows both fresh + stale-excluded. Found by config-check, not labeling (probe-primitive-first). Still gated on accumulation to ≥30/probe before graduation.
- [x] **llmx usage surfacing** — DONE (`llmx@62ce643`). Verify-before-build paid off: `usage_log.py` + `scripts/usage_summary.py` already recorded + rolled up usage; it just wasn't reachable. Promoted to `llmx usage` (cost/tokens by caller/cwd/model/provider, `--days`); PRICING+rollup single-sourced in `llmx/usage_report.py`; added `max_in`+`ctx_win`+`%used` for the context-window ask. Live signal: gemini-3.5-flash at 98%/1M context. `llmx usage --by model --days 7`.
- [-] **Candidate precision-weighted detectors: reasoning-action-mismatch + information-withholding** — MAST (NeurIPS 2025, verified) names these as top multi-agent failure categories; not yet explicit targets in our miner set (blindspot/drift/clash). Proposed as new detectors feeding the precision-weighting edge (②). Source: `research/2026-06-16-predictive-arch-rsi-loops.md` Addendum. NOT yet recurrence-gated — research suggestion, build only if a real instance shows in sessions. **[-] 2026-07-04 drain:** recurrence gate unmet after 18d — 0 new instances of either category (all 18 REASONING-ACTION MISMATCH entries are 2026-03/04; information-withholding has 0 instances ever), and the over_caution detector was just falsify-gated CONFOUNDED and cut. Resurrect on a real logged instance, per the item's own gate.
### [2026-06-14] Composer CLI integration — skills wired + nightly diff-review next
- [x] **Un-archive `/code-review` skill** — moved from `skills/_archive/` to `skills/code-review/`; scout scripts live in `agent-infra/scripts/` (not dead `meta/` paths). Default provider: `cursor` (composer-2.5). Replaces Claude Code vendor code-review plugin references in critique/execute closeouts.
- [x] **Skills wired to Composer CLI** — `/critique` (composer default on diff-closeout + close axes), `/sweep` (`--composer` Phase 3), `/observe` (Step 2b precision pass), `/verify-before status` (interpret step), `/improve maintain` (per-repo drift screen), `/execute` (phase-boundary premise probes, composer-2.5 not fast). `llm_dispatch` profiles: `composer_review`, `composer_screen` (fast).
- [x] **Critique axis overlap + cost tooling (2026-06-14)** — `standard` axes (`arch`,`gaps`,`correctness`,`contracts`) now all carry full-review mandate (structure+bugs); lenses differ, not exclusive territories. `just critique-cost` rolls up `.model-review/*.meta.json` usage. `scripts/cursor-skills-sync.sh` + friend-sync step mirrors key skills → `~/.cursor/skills/`.
- [ ] **Eval: critique/decide efficiency arms** — merged into `evals/critique_replay` full grid. Next: finish grid; scaffold `critique_outcome_link/`.
- [x] **Review dispatch consolidation (phase 1)** — closeout partition in `critique/SKILL.md`; `review_targets` in plan-close manifest; skill-usage-watch launchd killed; `critique_efficiency` tombstoned. flash35 demotion **reverted** 2026-06-14 — `just critique-health` shows gemini-3.5-flash 15% hallucination (lowest cosigner); keep in 2G+2GPT mix, never sole reviewer. Evidence: `decisions/2026-06-14-review-dispatch-consolidation.md`, `research/2026-06-14-decide-critique-cheap-gates.md`.
- [x] **Critique full-update Phase A0** — cross2/cross4/lens2/lens4 presets; AXIS_CELLS; review_gate preset triage; ROUTING_VERDICT scaffold. CLI default stays `standard`.
- [x] **VOI-sequenced review ADR** — `decisions/2026-06-15-voi-sequenced-review.md`; Composer scout before adjudication.
- [x] **Critique P0+P1 hardening** — outcome_link `linked_anchor`/`linked_file` tiers; rank → `escalation-recommendation.json`; cross-talk `cross_talk_degraded`; contradiction meaningful-token gate.
- [x] **Global review routing sync** — `~/.claude/CLAUDE.md` partitioned review pointers; `plan-review-gate.md` triage/VOI/closeout partition; `dependency-manifest.json` tombstoned orchestrator + fixed critique path.
- [ ] **Review dispatch (phase 2)** — finish `critique_replay` full grid → `ROUTING_VERDICT.md` JSON block; scout `--since-last-green`; `critique_outcome_link/`.
- [~] **skill-usage-watch collects nothing** — RETIRED 2026-06-14 (launchd killed per review-dispatch decision; half-loop, no consumer).
- [>] **Nightly per-repo diff review via launchd** — SUPERSEDED by scout `--since-last-green` extension per review-dispatch decision 2026-06-14.
### [2026-06-14] WIN 2 (RSI loop-closure) — mostly already built; residual is design-laden, not rushed
- [obs] VERIFY-FALSIFIED (5th this session): the leverage memo's WIN 2 claimed 4 detectors "dead-end with no actuator." Reading the code: `gov.py` ALREADY aggregates buildthenundo (gov.py:450) + advisory_noise/hook-roi (183) + gov_shrink retirement candidates (313) into `gov-report.md`, consumed by `/improve maintain` — that IS an actuator path. Recon overstated the gap (same pattern as the payload/LOC/retirement claims it also got wrong). Measure the principal.
- [>] **Residual: prediction REGISTRATION trigger (design-laden — do deliberately, NOT auto-stub).** The one genuine gap: `predictions.py` (shipped 2026-06-14) has a resolver + DUE-surface (drift-sentinel) but nothing AUTO-registers a prediction when a governance change ships → only 4 seed predictions. BUT its own docstring warns "a write-only prediction log is false comfort — the resolver is the point", so auto-appending skeleton stubs would CREATE the write-only noise it forbids (iatrogenic). The right trigger is a non-blocking registration NUDGE on governance commits (CLAUDE.md/hooks/rules/`[x]`) correlated to a real predictions.jsonl entry — design interacts with 2-day-old machinery; resolve the nudge-vs-architectural fork deliberately. Advisory-first (the fork Markus already settled). Evidence: `research/2026-06-14-leverage-hunt-rsi-system.md` WIN 2 + this session's gov.py/predictions.py read. **[>] 2026-07-04 drain:** superseded-by `register_implementations.py` — the fork resolved via pulse-tick auto-registration (ac208d4) + accept-gate auto-resolver (79a090e); ledger grew 4→28 with organic entries; the commit-time nudge was never built and is now redundant.
### [2026-06-14] SessionStart latency root cause: heavy MCP server imports (NOT agentlogs)
- [ ] **Lazy-import the heavy domain MCP servers (genomics/phenome).** Measured: agentlogs is NOT the cause (3.8GB DB but queries are 0.04s, and no SessionStart hook touches it — they just `cat` pre-written digests). Real cause: MCP spawns — `genomics_mcp.py` takes **5.25s just to import** `orchestrator.operator_views` (+fastmcp) at module load; genomics/phenome `.mcp.json` each load ~16 servers (incl biomedical/biomcp). agent-infra is light (5 mostly-HTTP global servers, no local `.mcp.json`). **Fix:** move heavy `from orchestrator… import` INSIDE the @mcp.tool fns so the process starts ~0.1s and pays import only on first tool call; and/or prune unused servers per project (`agentlogs query` on tool_calls.mcp_server). Lives in genomics/phenome — genomics has a LIVE session, coordinate there, don't blind-edit. Evidence: this session's `/usr/bin/time` probes + `research/2026-06-14-leverage-hunt-rsi-system.md`. **Revalidated 2026-07-04 (drain):** still open — `genomics_mcp.py:20-40` still module-level-imports 20+ orchestrator.operator_views symbols.
### [2026-06-14] Supervision is a direction VECTOR, single-sourced (fixes the autonomy sign-error)
- [x] **`supervision_taxonomy.py` — single-source correction taxonomy + direction vector.** The objective ("declining supervision") was a weighted SCALAR (`sli`) that summed opposite-sign corrections (WRONG→guardrail, TIMID→loosen) AND was blind to over-caution ("why ask"/"why not build it now" scored 0). Now: closed type set, each carrying a DIRECTION (reduce_error/raise_autonomy/grow_coverage/amplify_taste); `blindspot_miner.py`+`supervision-kpi.py` LOAD it (deleted duplicated BLINDSPOT_PATTERNS/CORRECTION_PATTERNS/BLIND_SEEDS — principle #9, drift-guarded by the test). Autonomy reading = conjunction (RAISE_AUTONOMY↓ AND errors/misses not rising), unexpressible as a scalar. Verifier (Gov-IDs were null): `scripts/tests/test_supervision_taxonomy.py` 22/22. Live: `--today` flags THIS session over_caution:2 (old SLI:0); 7d corpus = 21 over-caution events the scalar couldn't see. Extends/closes the [2026-06-14] RSI thread above (3796-3808: "standing pipeline = next build, proposed").
### [2026-06-14] genomics Modal-polling: state externalization (gated on probe) + Explore usage-shift
- [>] **Externalize genomics Modal pipeline state** — SUPERSEDED by genomics M2 stage-freshness workstream (probed 2026-06-14). Probe falsified the assumed cause: status tools aren't missing-dep/env-broken — `pipeline_status` does a LIVE 258-stage Modal `VolumeListFiles` crawl → rate-limited → TIMES OUT (the agentlogs "100% error"). genomics M2 (`docs/decisions/2026-06-14-stage-freshness-primitive.md`; commit `442bc035d` landed during the probe) is ALREADY building the per-stage cache that kills that crawl. A separate `state.json` populator = duplicate/collision → build-then-undo. Do NOT build it. Cross-model (Gemini+GPT) also rejected the watcher-subagent variant (P1 agent-theater). Evidence: `research/2026-06-14-subagent-offload-empirics-and-verdict.md`.
- [ ] **Residual (hand to genomics M2 owner): route the MCP STATUS TOOLS through the M2 cache** — M2 repoints `execution_plan`+`sample_remediation`, but `pipeline_status`/`sample_state` in `genomics_mcp.py` still call `view_sample_completion` LIVE = the 1490-calls/timeout agent-facing surface. Confirm M2 (or a follow-up slice) covers the status tools, not just the orchestrator internals. **Revalidated 2026-07-04 (drain):** still open — `view_sample_completion` still called live at `genomics_mcp.py:718`.
- [obs] OVER-ENGINEERING (averted): proposed a pipeline-watcher subagent; empirics + cross-model critique redirected to a native state-cache. The Explore usage-shift for the 57%-main-thread Read-hunting stays a behavioral recommendation — deliberately NOT new always-loaded prose (genomics is already OVER the 30k always-loaded ceiling per drift-sentinel) and NOT a build (the hookable form defers with the genomics fix).
### [2026-06-12] Stance-flip detector: Haiku predicate upgrade (dual-run, don't swap)
- [x] **Upgrade `stop-stance-flip-shadow.sh` from lexical markers to a Haiku semantic predicate** — Claude Code supports `type: prompt` hooks (Haiku, ~$0.001) but ONLY on tool events (PreToolUse/PostToolUse/PermissionRequest); Stop hooks stay `command`-type, so the upgrade is a Haiku call INSIDE the command hook (precedent: `source-check-haiku.py`). Design constraint: the hook is a fresh shadow instrument from another 2026-06-12 session — do NOT swap the predicate mid-measurement; ADD `haiku_hit` as a second logged column beside the lexical one so the shadow window yields a lexical-vs-semantic comparison for free, then promote whichever wins. Owner: whoever next touches the stance-flip instrument. **[x] 2026-07-04 drain:** shipped — the hook logs `haiku_hit`/`haiku_verdict` beside the lexical column (cc7c3ac, timeout fix 23e3776); shadow live-logging, 11/11 rows carry the column (last 2026-06-30, would_fire=3 vs haiku_hit=3). Remaining tail is the data-gated window-end comparison, not build work.
...
- [x] **Claude Code 2.1.1xx native-feature adoption** — closed 2026-06-12 same-day; per-item dispositions: (a) `disallowedTools` REJECTED — analysis skills already use the strictly-stronger `allowed-tools` allowlist; the skills lacking it are reference guides needing no bans; the commit ban is Bash-ARGUMENT-level (stays with commit-time guard hooks). (b) Stop-hook `additionalContext` ALREADY ADOPTED — 10+ shared hooks use it. (c) `context: fork` CONFIRMED live (probe-fork: forked execution, one-line result return) → applied to `sweep`; `upgrade` DEFERRED — interactive checkpoints inside fork unprobed; try after one real fork-mode sweep run. (d) `skills:` preload CONFIRMED (probe-cr subagent had the full medium-effort code-review protocol preloaded) — adoption opportunistic: needs custom agent defs (Agent tool exposes no `skills:` param). Registration gotcha for both: new skill/agent defs register on harness refresh with a LAG — retry once before declaring session-start-only. (e) 8K skills-index budget: measured 5,393/8,000 chars (67%) → threshold recorded in `.claude/rules/context-budget-principles.md §7`, NO lint until it binds (measure-before-enforcing); dynamic-workflows queue-runner caution folded into the same note.
- [x] **Mine `anthropics/defending-code-reference-harness`** (pushed 2026-06-02): packaged find→grade→judge→novelty→dedup→patch pipeline with executable oracle (ASAN PoC) + fresh-container verifier isolation + two-tier deterministic-grade→LLM-judge gating. Consumer: hutter Grinder/Dreamer verifier loop + evals graders, NOT the skills cluster. Mine `judge.py`, `docs/pipeline.md`, the grade↔judge contract. Source: .scratch/anthropic-github-survey.md. **[x] 2026-07-04 drain:** mined same-day into `research/2026-06-12-defending-harness-verifier-patterns.md` (grade↔judge two-tier contract, judge dedup verdicts, pipeline stage table) — checkbox was never ticked. Hutter consumer dormant (wind-down); no follow-on adoption item.
- [~] **skill-usage-watch collects nothing — diagnose**: retired 2026-06-14 — launchd killed; see review-dispatch decision. (Checkbox retagged 2026-07-04 stale-drain: body already said retired; the leading `[ ]` was inflating the open count.)
...
- **Observed:** the v1 plan **over-promoted**. The repo-access axis grep-proved 4 already-codified collisions the packet axes and I missed (the `/critique` Fable axis already shipped; hutter `v_calibration_by_tag` already exists *and is consumed* — the plan's "nothing consumes the residuals" was FALSE; intel `feedback_argument_primacy_over_source_tier` already captured source-credibility; the verifier-conditioned regime already a decision record). Plus 3 gate-fails (single-instance theses, duplicate taxonomy) and 1 venue error (`/session-end` is a human Raycast snippet, not agent infra).
- **Implemented (4 of 14):** `decisions/2026-06-10-silent-proxy-as-truth.md` (generalizes 2026-05-28's single face to 4 — dead-plane/projection/unit/platform); global `epistemic_discipline` point 8; `clean-verifier-evolver-pattern.md` move-8 revision (sign CAN invert across tiers — match family) + screen-unit + quota-stall anti-patterns + the generation-vs-self-verification clause; hutter `v_calibration_by_tag.avg_abs_miss` (mechanical band source, live-verified). Cut-list retained in `.claude/plans/f5afff31-promote-2day-gems.md` as calibration data.
- **Measured:** 14 proposed → 4 shipped + 1 absorbed clause; 10 dropped (4 already-codified, 3 gate-fail, 2 covered-elsewhere, 1 wrong-venue). Net: zero duplicate governance landed; the mining→gate→redistribute loop is the parallel-learning mechanism (not a push-MCP, which stays vetoed).
- **Reversibility:** high — 4 commits across 3 repos (agent-infra 9a32876+9444cfe, hutter 4b8b2a9, ~/.claude 37c7396); each a doc/view edit, `git revert`-able.
- **Status:** [x] shipped. No `/critique close` (plan too small — operator call). The repo-access-axis-on-PLAN-reviews discipline (`critique/SKILL.md` Known Issues 2026-06-10) paid off a third time: it caught the false "nothing consumes" premise the packet axes asserted as fact.
...
- **Status:** [~] retired 2026-06-24 — CUT (not promoted) + infra eradicated; see the [2026-06-24] RESOLVED/RETIRED entries below and `decisions/2026-06-24-risky-diff-review-shadow-retired.md`. Was: proposed (SHADOW); promote/cut criterion's correlation half falsified by negative control. Gov-ID `hook:risky-diff-review-shadow`, blast_radius local.
...
- **Proposed fix:** [architectural] Stamp caller/script/skill ID into each `llmx-usage.jsonl` record so billing is programmatically attributable per-spender. Per-call usage is already logged (model, effort, tokens, latency — see `llmx info` /
`~/.claude/cache/llmx-routing.json`); the caller field is the named gap the agent itself flagged mid-session ("add caller attribution to llmx's usage logging — that's the real leverage"). Checkable: `jq` rollup by caller becomes possible. Touches `~/Projects/llmx` (separate repo) → propose, don't auto-apply.
...
- **Status:** [x] SHIPPED — verified 2026-06-12 (maintain finding-drain). The proposed fix exists: `cmd_index` installs a hard SIGALRM backstop at +60s/source (`cli.py:197` `_signal.signal(SIGALRM, _hard_deadline)`; docstring `cli.py:48`), preempting a pure-Python parse hang the sqlite progress handler can't reach.
...
- **Status:** [ ] proposed — residue sharpened 2026-06-12 (maintain). The prune **mechanism is fully built and safe**: `agentlogs prune` (`cli.py:451 cmd_prune` → `prune.py plan_prune/apply_prune`), dry-run by default, takes the indexer flock, VACUUMs to reclaim. What's unwired is **auto-invocation** — no launchd job, no post-index call, so the one-time manual prune (14GB→3.2GB) has regrown to 3.4GB. Deliberately NOT auto-wired by this loop: `keep_days` is a retention-policy judgment on the historical ledger (constitution: "the git log is the learning"), and 3.4GB is non-acute → wants a human retention-decision, not an autonomous default. Decision-ready: pick `keep_days` (≥60 to match the read window) then either a post-`cmd_index` call or a daily launchd job.
...
- **Status:** [ ] proposed — acute driver removed 2026-06-12. The chronic repeat-failures that motivated this (agent-infra `078ec1d8` 15×, genomics `15bb89f2` 29×) were a *bug*, not legitimately-poison sources — fixed at root by migration 005 (self-FK index, `9956a21`); they now import in one pass. Residual scope is the genuinely-pathological case only (oversized retired-vendor junk that can never parse in-budget), which is rare → stays low-priority. The (path, sha) quarantine is still the right general fix if a real poison source recurs.
...
- **Status:** [>] superseded 2026-06-12 — resolved the *opposite* way the finding proposed. agentlogs deliberately ships INSIDE the agent-infra wheel via `[tool.hatch.build.targets.wheel.force-include]` (`pyproject.toml:53`, no own dist); the dual-package was NOT dropped. The footgun is now a *documented workflow* not a latent bug: `session-forensics.md` rule states "after editing `src/agentlogs/`, `uv sync --reinstall-package agent-infra`" (and `--reinstall-package agentlogs` is a silent no-op). Knowable, not silent → superseded by the rule, not fixed as written.
...
- **Root cause:** system-design
- **Status:** [x] FIXED 2026-06-12 (agent-infra@9956a21) — **lock-wait hypothesis was WRONG.** The indexer was already single-flighted (`locks.indexer_lock` flock on `agentlogs.db.indexlock`); no concurrent writer existed. Probe-measured: parse of the 42MB file = 1.1s, the 992KB file = 0.0s; ALL time was inside the events `executemany` (stack sample: 100% `sqlite3BtreeNext`). Actual root cause: `events.parent_event_id` is a self-referential FK with no child index, and the UPSERT's UPDATE branch reassigns `event_id` (the parent key) — with `foreign_keys=ON`, each conflicting row full-scanned 567K rows to verify the old key unreferenced. Active sessions take the conflict path for every prior event on every WatchPaths re-import → 311-event source >10 min → 180s watchdog kill → permanent red. Fix: migration `005_events_parent_event_index.sql` (index the FK child column + ANALYZE). Post-index, the 15×-failed source imported on the first straddling run (indexer_runs 8452). Lesson: "lock-wait" was asserted without a stack sample — one `sample <pid>` falsified it in 5s.

### [2026-06-12] [obs] SUPERVISION AUDIT: 1.8% wasted (89 sessions, 932 msgs) — near-zero, no promotable findings
- **Period:** 1 day, 89 sessions, 932 user messages. Wasted = 17 (1.8%, target <15%).
- **Breakdown:** NEW_AGENCY 915 · CORRECTION 15 · RUBBER_STAMP 2 · BOILERPLATE 0 · RE_ORIENT 0.
- **context-resume (10):** measurement artifact — all are the harness-injected "Continue from where you left off" compaction-continuation string, not user corrections. Already addressed (checkpoint.md + CLAUDE.md auto-resume rule + UserPromptSubmit hook); residual is sessions that never compact (no checkpoint generated). Recurring observation, see log lines 1372/1500/1536/1670. The extractor mis-classifies these as CORRECTION, inflating the wasted total — a classifier refinement opportunity, not a behavioral fix.
- **repeat-instruction (4):** all singletons. Sharpest: session c54269ad — user asked 3× for a Fable5-throttle detector; agent over-deliberated on the counterfactual-detection difficulty instead of shipping a 1/10-complexity probe. Task-execution failure, not an automatable supervision pattern. Below recurrence threshold.
- **Status:** no promotion warranted. Digest at artifacts/observe/digest.md.

### [2026-06-12] [obs] EVAL: effort buys design-SYNTHESIS, not critique — first quality separation in 6 dispatch evals (anim-workbench)
- **Session:** anim-workbench 83f551c2 (preregistered: `.claude/evals/2026-06-12-fable-effort-architecture/`, prereg committed before dispatch)
- **Evidence:** Fable `--effort low` vs `high` on a byte-identical UNGATED architecture challenge (same fork sha; high arm ran a day earlier and its winning design had already shipped, making mechanical grading anchors available). Low = 0.34× output tokens, and on CRITIQUE quality matched high: 4/4 correct cosigns, 0 false anchors (5/5 grader-verified at the fork sha), calibrated NOOPs, 2 novel verified proposals high lacked (anonymous plot binder restoring structural-diff equality; generic expr walker). High won exactly one thing: the SYNTHESIS of novel structure (orthogonal content⊥visibility text model; low folded reveal into the content union, making the target composition — a typing counting-number — inexpressible). All five preregistered predictions hit, incl. the discriminator. Consistent with the prior 5 evals where gate-bounded work tied across all lanes/efforts.
- **Generalization (screening-grade):** the effort dial separates on task SHAPE, not difficulty — verdict-against-existing-structure is effort-insensitive; novel-structure-from-open-hole is where depth pays. Cross-model leg (GPT effort on critique) is unmeasured extrapolation.
...
### [2026-06-14] [x] EXIT-CODE-AS-REPORT-STATE: audit_corpus_sync exits 1 on drift-detected, RSI sweep misreads as crash (drift, incremental)
- **Drift run:** artifacts/observe/2026-06-14-drift-incremental/ (delta-since-21d-run pass, 2 batches, deep_review per-batch; candidates.jsonl). Window: sessions since the 2026-06-13 21d drift run.
- **Evidence:** 3 distinct agent-infra sessions (2b8ac808, bcaa3f63, cc48122e — all verified on disk) had the `com.agent-infra.audit-corpus-sync` launchd job flagged "crashed" by the RSI maintenance sweep. **Code-confirmed at primary source** (not Gemini-attributed): `scripts/audit_corpus_sync.py:580` returns `1 if drift_total + relation_drift_total > 0 else 0` — unconditional, not gated on a `--strict` flag; `--json` help (line 462) explicitly documents "non-zero exit on drift." The launchd plist invokes `audit_corpus_sync.py --json --corpus-root /Users/alien/Projects/corpus`, so a *healthy* run that merely detects drift (the job's entire purpose — surfacing verdict↔annotation drift for human triage) exits 1, which a launchd-crash sweep reads as a process failure → alert fatigue + repeated manual verification.
- **Failure mode:** system-design — conflates a semantic report state (drift found, needs triage) with an execution-crash exit code. Same anti-pattern class as the silent-proxy/principal-check confusion but inverted (a *success* misreported as failure).
- **Proposed fix:** [architectural] under launchd/`--json` mode, drift-detected is exit 0 with a structured `drift_total` field for consumers; reserve non-zero exit strictly for fatal execution failures. (The `--strict`/CI use case that *wants* non-zero-on-drift becomes an explicit opt-in flag, not the launchd default.)
- **Root cause:** system-design
- **Status:** [x] implemented — `--strict` opt-in added; default (the launchd `--json` invocation) now exits 0 on drift with `drift_total` in the JSON, reserves non-zero for fatal failures. Verified both exit paths (0 default / 1 `--strict`) + kickstarted the launchd job → status cleared 1→0. Single-variable, agent-infra-local. Implemented by session bcaa3f63 (the drift run that promoted it ran concurrently).

### [2026-06-14] [obs] DRIFT (incremental) — hutter remote-fleet drift + dropped proposals; phenome eval-rigor; superseded items
- **Drift run:** artifacts/observe/2026-06-14-drift-incremental/candidates.jsonl + drift-digest.md. Inventory-gated: the 21d wide pass ran 2026-06-13 and fully promoted; this incremental pass dispatched only the ~280 sessions since, against prior coverage, to avoid re-discovering yesterday's findings.
- **Rising friction / convention drift (hutter, single session 897a2209, below 2+ gate → obs):** remote Hetzner box config diverged up to 58 lines from M3 (stale MemoryMax killed legalizer probes, stale CONSTRAINTS.md decode-limit) undetected 12h until a mechanical drift-checker was built → cheap `just sync-box` pre-flight git-sync is the proposed lever; bespoke SSH params (accept-new blocking IP-recycled boxes, ConnectTimeout=5 dropping markers on saturated CPUs) drift across fleet_sweep/fleet_watch/idle_reaper → centralize into one helper. hutter-local; route to hutter, not agent-infra harness.
- **phenome eval-rigor (adfa97e4, single session, route to phenome):** closed-world DB-absence labels penalize correct real-world biology (tramadol/CYP2D6, SciFact NEI) → "only affirmative DB-attested defeaters are valid negatives; local absence = UNASSESSABLE." Eval-design, not harness.
- **Superseded (NOT new — verified against prior work):** prompt-file wrapper-preamble derailment is already documented in the observe SKILL.md (verbatim `--prompt-file` rule) and this run used the inert-data fence; unconsumed-research backlog was RECONCILED 2026-06-13 (`orphan_findings.py` built). Proposals-pending drop (multi-parent crossover / loop-health monitor) is days-latency, same class as the known proposals-backlog hygiene — hold, not a new build.
...
### [2026-06-13] [x] RECONCILIATION: triaged 31 un-harvested trending-scout verdicts (orphan_findings catch-up)
- **Trigger:** Discovered /improve harvest never read research/trending-scout-* memos → 3 months of adopt-grade verdicts bypassed the loop. Built `scripts/orphan_findings.py` (deterministic memo-cite ratchet, sibling to orphan_check.py) to detect + this one-time catch-up.
- **Scope:** 11 trending memos, 32 actionable verdicts; 31 in 7 un-cited memos. Citing full names marks them routed so the detector clears: trending-scout-2026-03-19, trending-scout-2026-03-26, trending-scout-2026-03-29, trending-scout-2026-04-03, trending-scout-2026-04-05, trending-scout-2026-04-17-clojure-mcps, trending-scout-2026-06-11.
- **Disposition (re-verified against deferred-tracker + git + actual stack):** net-new live `[ ]` = **0**. Breakdown: DONE — `effort` frontmatter, hook `if:` conditionals, `--bare`, `--safe-mode`, additionalContext-on-Stop (5 stop hooks already emit it); EVALUATED-REJECTED — `disallowedTools`, `initialPrompt`, `CwdChanged/FileChanged` (deliberate defers, deferred-tracker 2026-03-23/04); MEMO-ONLY (Extract/Watch, study-not-build) — claude-plugins-official, hermes-agent, chrome-devtools-mcp, superpowers, OpenViking, hindsight, cocoindex, Gemini context-injection, MS governance toolkit, OpenSpace, FastMCP Code Mode, MS Agent Framework, pydantic-deepagents, clojure-mcp-light, harness-paper-cluster (→ covered by RSI sweep); N/A-TO-STACK — LiteLLM supply-chain (only in research memos, not a dependency); OWNED-ELSEWHERE — jfrog/agent-belt eval-against-binary (→ evals repo 2026-06-13-frontier-agentic.md). The one live item (hook `if:` paths, 2.1.176) was promoted separately this session.
...
- **Evidence:** The `corpus` CLI was broken all session (uv-tool editable `.pth` pointed at the moved `agent-infra/scripts/corpus/packages/corpus-core`; live source promoted to `substrate/packages/corpus-core`, same as evalcore). A parallel session found it via real usage. My loop ran 6+ health ticks reporting "all green / verified noop" — and every check (hooks-smoke, launchd-exit, freshness, doctor, orphan-check) WAS green, because none exercised the corpus CLI or validated uv-tool editable pointers. `audit-corpus-sync` stayed exit-0 (library `corpus_core` still imported; only `.cli` was gone), so no launchd signal either.
- **Failure mode:** P8 silent-proxy-as-truth — let "my checks are green" stand for "the system works" when the checks don't cover the surface. Recurrence of the check-the-actual-traces class (gov-intake adfa97e4, genomics 1c094e24).
- **Fix shipped:** [check] `doctor.check_uv_tool_editables()` (7956969) — validates every uv-tool editable target dir exists; warns on stale pointers (the moved-package class). Verified 6/6 resolve + negative control. Deterministic, catches all editable tools, not just corpus.
- **Root cause:** coverage hole — synthetic health checks never invoke the real CLIs; only live execution (or now this deterministic pointer check) surfaces a moved-package break.
- **Status:** [x] deterministic half shipped (uv-tool-editable check)
- **Status:** [obs] behavioral residue — "health-green ≠ system-works"; live tool exercise / smoke is the only catch for CLI-level breakage a library-import check can't see. Consumer: recurrence→rule, not a per-item build. [[live-execution-is-the-integration-verifier]]
- **Reactive miner (user-directed "check the logs for what doesn't work"):** the failure WAS in `tool_calls.status='error'` for days; observe never queries it. Tried the deterministic miner — 4 probes show the cheap version is a noise trap (path-substring, by-design-nonzero linters like ruff, time-bounded-outage dilution, no clean dead-stretch); `tool_calls` records status but not error TEXT. Precise version needs an `events`-join + launch-failure pattern bank → escalated to `decisions-pending/2026-06-14-tool-failure-miner-from-logs.md`. Did NOT ship a noisy detector (crying-wolf = worse than none).
- **Reactive miner — BUILT (user directed "update the skill and run it"):** the precise version works — filter to real crash SIGNATURES (Traceback+raised import / shell command-not-found / entry-point shim) BEFORE grouping, which beats the 4 noisy probes (the launch-failure text never appears in by-design-nonzero output). Shipped as `/observe failures` (skills@0f98c1d): Tier1 deterministic `scan_tool_failures.py` ($0, events-join, cluster by root cause) → Tier2 Haiku triage → Tier3 escalate big clusters. Wired into `/improve maintain` rotation (weekly) so it's not dead infra. decisions-pending proposal retired (built). Status: [x]
- **First-run findings from `/observe failures` (dogfooding):**
  - **`missing-module:duckdb` ×46 / 10 days** — BIG. Tier-3 root-cause: 34 bare-`python3` + 10 `uvx python3` (isolated, no project deps) vs 14 `uv run`, concentrated in `intel`/`intel-harness`. Invocation anti-pattern (`uvx python3 <<EOF` / bare `python3` instead of `uv run python3`), not a one-off. Status: [ ] proposed (intel-scoped → propose; candidate: extend bare-python guard to catch `uvx python3` for project scripts).
  - **`broken-cli:corpus (corpus_core.cli)`** — the original incident; [x] fixed (editable check 7956969 + cross-session reinstall).
  - **`just commit-mine` trailer-parse bug** — commit-body lines (`Diagnosed:`, `Research:`) executed as commands at recipe line 126 → `command not found`. Status: [ ] proposed (agent-infra-local, low-sev; heredoc/quoting fix in the recipe).

### [2026-06-14] [LEVERAGE / outer-loop]: Loop watched proxies + buried output → ranked Top-N priorities digest
- **Source:** user `/leverage` on the outer loop — "this session's observe/improve didn't find/fix much; it should mention after each run the top 10 priorities to plan."
- **Diagnosis:** the autonomous loop found 1 real thing (the agentlogs reaper) but missed corpus (dead for days) + duckdb (46×) because it watched INWARD proxies (hooks/launchd/indexer) not OUTWARD work-state, reported "verified noop / all green" (false comfort — the Constitution's own drift signature: supervision dropped, errors undetected), and buried what it did find in logs (no ranked surface). propose-work.py existed but its top-19 "URGENT" were stale-agent/uncommitted noise.
- **Fix shipped:** `scripts/top_priorities.py` (aa59976) — ranked cross-repo plannable digest from high-signal sources only (broken tools via observe failures, pending decisions, open [ ] findings, REAL doctor fails, stale plans), proxy-noise suppressed, missing-dep cluster collapsed. Wired into `/improve maintain` as the per-run headline output (skills@7a7fd6c); writes gitignored `PRIORITIES.md`.
- **Status:** [x] core shipped (outward sensor + ranked Top-N output, levers B+C+A).
- **Status:** [obs] remaining levers (next rinse): D noop-streak→coverage-alarm (a dry loop = wrong coverage, not perfect system); E findings→auto-draft plans; F active-probe tier (loop RUNS tools, generalizing failures mode). [[state-externalization-lens]]

### [2026-06-14] DECISIONS CLEARED (3 pending → dispositioned with Markus)
- **demand-test-the-conductor → PASSED, keep + SessionStart surfacing.** Pre-registered rule (≥5 real-work `/improve maintain` ticks/14d) met this session (reaper fix, supervision audit, failures mode, top_priorities, finding-routing). Adopted the test's load-bearing insight regardless: PRIORITIES.md now surfaces at SessionStart (`priorities-surface.sh`, wired) + regenerates daily via drift-sentinel — so priorities show up in sessions that already happen, zero dependence on typing /loop. Status: [x] resolved.
- **loop-resume-after-autocompact → verify organically (Markus: "make the loop shorter, verify faster").** Noted the mechanism nuance (auto-compact is context-size-driven, not clock-driven) — so shorter interval doesn't trigger compaction faster, but more fires = more chances to catch a real post-compact resume in the wild. Go-forward: loop re-arms at 30m (was 60m); the running `/loop improve` will organically verify resume when a heavy session compacts. Candidate fix if it fails: SessionStart(compact) hook re-issues the loop prompt. Status: [obs] watch — resume-after-compact (no sacrificial run; checkpoint net already protects state).
- **first-party-search-inventory → DEFER (recurrence-1 of that surface, below the 2+ bar for a 3-project rule).** No global-rule edit on thin evidence; the [obs] at improvement-log:3737 keeps tracking. Resurfaces if a 2nd first-party occurrence lands. Status: [-] deferred (not rejected — bar not yet met).

### [2026-06-14] Loop-resume test COMPLETE — compaction-survival PASSED (closes the [obs] at :3771)
- **Test:** does a `/loop` ScheduleWakeup wakeup re-enter and continue the loop ACROSS a context compaction that clobbers the conversation? Markus drove it ("Can't you try autocompacting here now?") — manual `/compact` substituted for auto-compact (auto is context-size-driven, can't be forced on demand; mechanism-equivalent for the resume question).
- **Result: PASS (both halves).** (1) Unattended self-continuation — prior tick, wakeup fired with zero user input (committed fdc04d7). (2) Compaction-survival — THIS tick: session `53a9f3e3` underwent a real manual compaction (compact-log precompact 11:50:55 → post_compact 11:53:14, agent-infra cwd; PreCompact rewrote checkpoint.md with my verbatim content), then the `/loop improve` wakeup re-entered post-compaction with only the `/compact` command between (no user steering). Loop continued the tick.
- **Honesty note (P8, both directions):** the pre-registered verifier `grep 9547ab33 compact-log` returned 0 — but that predicate used the wrong ID. `current-session-id` (9547ab33) is stale shared-state; this conversation's real transcript/compaction ID is `53a9f3e3` (sole transcript carrying the RESUME-TEST marker; no 9547ab33 transcript exists). Fixed the proxy, ran the principal check (am I the compacted session? did the loop resume?) — PASS. Did NOT report FAIL on a broken proxy, nor route around it to a PASS.
- **Mechanism that made it survive:** the wakeup PROMPT carries the test/continuation instructions (delivered fresh by the harness at fire time, not stored in the clobbered context) + PreCompact `checkpoint.md` re-orients. Both load-bearing. No SessionStart(compact)-reissues-loop hook needed — the candidate fix from :3771 is unnecessary; the existing nets suffice.
- **Status:** [x] resolved — loop is verified unattended-safe across compaction. The standing `/loop improve` maintenance loop can run all-day through heavy compacting sessions.

### [2026-06-14] "do all 3" top priorities — only 1 was a real bug; the dig fixed the DIGEST
- **Source:** user "ok do all 3" on the surfaced top-3 (duckdb missing-dep, RoutingInput ImportError, corpus missing-modal). Dug each to ground truth before touching code (probe-first) — and 2 of 3 were not actionable bugs:
- **#3 corpus `No module named 'modal'` → ALREADY FIXED today** by substrate `788913b` (2026-06-14 10:48, "[corpus-core] Add --reparse + actionable marker-modal dep error"). The `import modal` at `pdf_marker_modal.py:34` is already lazy (inside the marker path); the CLI no longer dies on missing modal. The 4 failures predate the fix. **User's hunch ("phenome might have fixed corpus recently") was correct.** Status: [x] verified-fixed-upstream (no action).
- **#2 `RoutingInput` ImportError ×34 → PHANTOM (digest artifact).** `scan_tool_failures` lumped every `cannot import name X from Y` into one coarse `import-error` bucket sampled by the most-recent hit, so 34 assorted moved-symbol breaks across genomics/intel showed as "RoutingInput ×34". Only **2** were actually RoutingInput (genomics `prs_validation_core` — class is `RoutingInputs` plural; the 2 were ad-hoc/non-bash singular typos, no committed caller, main repo clean). **Fix shipped (B):** sub-key the cluster `import-error:<symbol>@<module>` (skills@08bb5d6) → RoutingInput now shows its true size (2/1d, off the top); honest top-5 is all real missing-module clusters. Status: [x] miner precision fixed; the "bug" was the digest crying wolf.
- **#1 duckdb `No module named 'duckdb'` → REAL & ongoing**, but multi-cause (agentlogs, real raised-signature only, since May-26): 16 bare-`python3` + 3 `uvx python3` (invocation: system/isolated python lacks deps — catchable) + ~10 legit `uv run python3` in dirs genuinely missing the dep (evals/extraction_bakeoff [settled bakeoff], ibkr exports — transient, not a committed bug). **Fix shipped (A):** `pretool-uv-python-guard.py` (skills@69e4c0c) wired global (~/.claude@f6b5860) — blocks bare-python at start + `uvx python` without `--with` (shaped as invocation), warns compound bare-python, passes `uv run`/`uvx --with`/mentions. Replaces the inline guard that only caught start-of-command + broke on escaped quotes. Status: [x] invocation subset guarded; the uv-run-missing-dep subset is transient/settled (not actioned).
- **Dogfood lesson (hook FP, captured as a guard test):** matching a python invocation ANYWHERE in a command string FP's on MENTIONS — the first cut self-blocked a quoted JSON test payload AND its own commit message. Fix: blank quoted substrings before matching + require invocation-shape (flag/heredoc/path/EOL), not a prose mention; reserve BLOCK for start-of-command + uvx-without-with, WARN the FP-prone compound case (P3 measure-before-enforce). [[live-execution-is-the-integration-verifier]] — the wired hook caught its own FP in-session.
- **Meta (the real win):** 2 of 3 "top priorities" were digest artifacts (1 stale, 1 coarse-cluster phantom). The loop's headline output was crying wolf; the dig's highest-leverage output was tightening the miner, not three code fixes. Remaining miner refinement noted: `fails` counts events not distinct tool_calls (event-multiplication) — [obs], fix if it distorts ranking.
- **Status:** [x] all 3 dispositioned (1 guarded, 1 phantom-fixed, 1 already-fixed) + miner precision + hook-FP lesson.

### [2026-06-14] RSI META: every user flag = a loop coverage gap → convert to a detector (the directive)
- **Source:** Markus, verbatim: "RSI: why did you not find this? every time I mention something to you ... you should ask yourself the question ... and metaimprove a way for the next loop to find stuff like it in the future." Twice today (corpus, prior-context) I treated his flag as a thing-to-fix, not as a labeled training example of a loop miss. The reflex must be ARCHITECTURAL (instructions are 0% reliable), and the objective is the constitutional one: the RATE of "why didn't you find X" → declining supervision.
- **Root cause (verified, not hypothesized):** the loop's own supervision instrument was BLIND to exactly this class. `supervision-kpi.py` CORRECTION_PATTERNS = `^(no|not that|wrong|stop|undo|revert)` (start-anchored, blunt). His actual highest-signal flags this session — "SOOO why didn't you find this bug?", "why don't you check the system", "why did you not find thid" — ALL scored `corrections:0`, as did phenome corpus session adfa97e4. You cannot reduce supervision you cannot measure: no gradient. AND none of the RSI machinery (supervision-kpi, observe/extract_supervision, reflect.py) is SCHEDULED — all interactive-only (only reflect-eval, a one-shot, ever fired). So flags never reach the loop, never convert to coverage.
- **Tool lessons (Markus caught both reflexes live — "use a better tool, you can install anything" + "is model2vec better than emb?"):**
  - **L1 — semantic/pragmatic predicate → DON'T hand-roll regex.** "Is this message a blindspot flag?" is intent classification; reach for an LLM classifier or embeddings. AND check in-house tools FIRST: I almost added `model2vec` when `~/Projects/emb` (Markus's mature embed tool — gte-modernbert-base, hybrid, rerank, all-pairs `pairs`) already exists. Adding a redundant dep when the better in-house tool exists IS the prior-context-blindness pattern, one level up.
  - **L2 — embedding SIMILARITY = topic, not pragmatics.** Plain sim-to-seeds FAILED (margin −0.081 with emb's strong model; "did you check the git log" ≈ "can you check the tests" topically but opposite pragmatically). CONTRASTIVE scoring (blind-centroid − normal-centroid) fixed it: 13/15 clean, the FP "check the tests" went 0.660→−0.308. Use contrastive for intent-like distinctions.
- **Validated detector stack (probe-confirmed, NOT built yet):** regex Tier-0 ($0, obvious forms) → emb contrastive ($0 local, paraphrase-robust, runs in emb's env to avoid dragging torch into agent-infra) → LLM (Haiku/Flash) escalates only the near-zero band → `emb pairs` clusters recurring flags. Then: standing launchd job (fix the "nothing scheduled" gap) → surface flagged misses + SLI trend at SessionStart → LLM-convert each recurring flag to a candidate detector → improvement-log/decisions-pending. The blindspot-flag RATE is the measured objective → drive to zero.
- **Shipped so far:** `supervision-kpi.py` now HAS a `blindspot_flags` counter (regex Tier-0, weighted 5× in SLI) — the instrument can finally SEE the class (this session 0→3 flags; 14 sessions across all repos now light up). The emb-contrastive upgrade + the standing/convert pipeline are the next build (proposed; integration approach = run-in-emb-env, pending Markus's nod).
- **Status:** [ ] FOUNDATION shipped (instrument no longer blind); emb-contrastive detector + standing schedule + convert-to-detector loop are the open build. This entry IS the convert-to-coverage for "the loop doesn't see what the human catches."

### [2026-06-14] RSI blindspot loop — BUILT, INTEGRATED, and the tool chosen by data
- **What shipped (the full diagnose→generate→integrate loop, not a stray script):**
  - DETECT: `scripts/blindspot_miner.py` (emb-contrastive, runs in emb's env, $0 local) + `com.agent-infra.blindspot-miner` launchd (daily 06:50) + `blindspot-surface.sh` SessionStart + `just blindspot`. First run: 41 flags/2d; dominant cluster across genomics+intel+agent-infra = prior-context blindness.
  - GENERATE/INTEGRATE: `/observe blindspot` mode (diagnose) + `/improve harvest 2h` (digest as first-class source) + `/improve maintain` "Blindspot → detector" rotation row (the CONVERT step: cluster → propose the detector that would've caught the class → improvement-log/decisions-pending). Closes generation-without-consumption — Markus caught that the miner was surfaced but not LOOPED ("is it integrated into the diagnose→generate→integrate loop?").
- **Tool decision, data-driven (Markus pushed: "is it actually better than regex/fuzzy? you can install anything; is model2vec better than emb?"):** benchmark on a labeled set w/ hard negatives — regex 43% recall (misses every paraphrase); fuzzy/rapidfuzz hits a LEXICAL CEILING (50% at safe precision OR cries wolf); emb-contrastive (blind-centroid − normal-centroid) is the ONLY method catching semantic paraphrases at precision (13/15). Used the in-house `~/Projects/emb` (gte-modernbert-base), NOT a new model2vec dep — reaching for the existing tool IS the prior-context discipline. Pulled the rapidfuzz experiment from supervision-kpi (marginal + wrong home); it landed in emb instead as `emb pairs --fuzzy` (emb@a674874, the natural home for char-fuzzy dedup).
- **Lessons (the metaimprove):** (1) semantic/pragmatic predicate → reach for the right tool (contrastive embeddings / LLM), never hand-rolled regex; check in-house tools FIRST. (2) embedding SIMILARITY = topic, not pragmatics → CONTRASTIVE for intent. (3) the loop's headline outputs must be CONSUMED by the loop skills, not just surfaced.
- **RSI loop health audit (Markus: "are all your RSI loops firing correctly?"):** all 12 `com.agent-infra.*` launchd jobs last-exit 0; daily jobs confirmed fired today via `.out` mtimes (drift 06:35, vendor 06:00, gov 05:45, test-health 05:43 — the Jun-3 `.err` was last-stderr-write, not a dead loop); only benign stderr (codebase-map `\m` SyntaxWarning [minor, fix], agentlogs lock-guard by design). blindspot-miner verified via launchd kickstart (real PATH/uv/emb env), not just manual run.
- **Side finding:** skills description index = 9071 chars > ~8000 Codex ceiling (was 5,393 on 2026-06-12) — now BINDS. [obs] trim the skills index (37 descriptions); kept my observe edit lean rather than add to the overage.
- **Status:** [x] blindspot loop built + integrated + tool-validated + health-audited. [ ] OPEN convert-target: prior-context detector at the propose/diagnose boundary (the #1 cluster the miner names; extends inventory-dispatch). [obs] skills-index over budget.

### [2026-06-14] CONVERT done — prior-context front-load hook (the #1 blindspot cluster → architecture)
- **Closes the open convert-target from the entry above.** The blindspot-miner's dominant cluster (across genomics/intel/hutter/agent-infra) is the human having to say *"check the git logs about what we did first"* / *"did somebody research this already?"* / *"how did you forget that?"* — the main loop proposing/diagnosing WITHOUT reading prior discussion. This is the loop's convert step realized: cluster → the detector that would have caught the class.
- **What shipped:** `skills/hooks/userprompt-prior-context.py` (UserPromptSubmit) — the MAIN-LOOP analog of `pretool-inventory-dispatch.py`. inventory-dispatch front-loads "what's already done" when a *subagent* is dispatched; this front-loads it when the *user's prompt* is a propose/build/diagnose/status-of-X request whose keywords overlap real prior work (git-log ≤30d + research-index rows + research/decisions filenames + ideas.md lines). Intent-gated (regex runs before any I/O — non-propose prompts pay one regex and exit), match-gated (fires only when prior work exists — precision comes from the match, not the intent), session-deduped (a focused session won't re-inject the same topic every turn — that's how advisory hooks train themselves to be ignored), fail-open. 12/12 selftest (hermetic HOME, no litter) + live probe against the real repo surfaced the actual blindspot commits. Wired global as the 4th UserPromptSubmit hook. Commits: skills@a174cf0, ~/.claude@ee66cd6.
- **Boundary choice (explored 5, the phase-state artifact):** (1) UserPromptSubmit front-load ✅ — earliest boundary, true main-loop analog, deterministic $0; (2) PreToolUse:Write on proposal artifacts — later + noisier (every improvement-log append), deferred complement; (3) Stop-hook post-hoc — already covered by blindspot-miner at coarse grain; (4) Haiku intent-judge — measure deterministic first (P3); (5) CLAUDE.md instruction — *this is the failed status quo* the cluster records (P1). Self-contained on purpose: hooks must be independently fail-open, so STOP/scan logic is NOT shared with inventory-dispatch (a shared-module bug would kill both); surfaces also legitimately differ (user prose vs structured dispatch prompt). If keyword lists measurably drift, extract then.
- **Pre-registered success metric (the declining-supervision objective, not vibes):** the prior-context cluster's flag-rate in `.claude/blindspot-digest.md` should DECLINE over the next ~2 weeks; `hook-trigger-log.sh` (label `prior-context`) measures fire volume + lets us judge precision (false-fires = surfacing context I didn't need). **Re-check ~2026-06-28** via `just blindspot` (cluster shrunk?) + `just hook-telemetry` (fire rate sane, not noisy). If it fires constantly without changing behavior → demote/tighten. If the cluster doesn't shrink → the boundary was wrong (escalate to option 2). This is the loop self-assessing its own convert.
  - **BASELINE 2026-06-14 (the denominator — else "did it shrink" is vibes):** prior-context/rediscovery is the **#1 theme** in the digest — strict-RGX matches 5 of the 25 shown flags, but the *semantic* cluster is ~half of them ("how did you forget that", "redoing work already done", "we talked about this", "check the git log + memos so you don't repeat mistakes", "is there a better tool"). The strict-regex undercount IS the reason emb-contrastive (not regex) defines the cluster — so the re-check is the **emb miner's** view, not a grep. **PASS** = on 2026-06-28 prior-context no longer dominates the top flags AND `grep prior-context ~/.claude/hook-trigger-log*` shows it fired (proving the hook had chances to help, so a flat cluster would mean it didn't work — not that it never ran). **No bespoke one-shot scheduled:** the daily blindspot-miner + SessionStart digest surface ALREADY re-measure this every day (that's the outer loop — Native-First: don't add a launchd job duplicating a daily measurement, unlike the risky-diff one-shot whose git-history scan no daily job computes).
- **Status:** [x] convert shipped + wired + verified. [ ] pre-registered metric OPEN until ~2026-06-28 (cluster-shrink + fire-rate check).

### [2026-06-14] skills-index [obs] was a MIS-MEASUREMENT — real overage was intel-local, now fixed
- **The [obs] ("skills description index 9071 > ~8000 Codex ceiling — trim the 37 descriptions") was measuring the wrong population.** Verified before trimming (don't fix a phantom — same lesson as the "RoutingInput ×34" phantom earlier today). Three distinct skill populations exist; only one is a loaded index:
  - `~/Projects/skills/*` (SOURCE dir, 37 skills) = 9662 chars — this is the dev source, **no agent loads it as one index**. The `[obs]`'s "9071" came from here. Meaningless as a budget number (sums exported general skills + 18 domain skills like oura-ring/neurokit2/x-api/census-data that never co-load).
  - `~/.claude/skills/*` (global EXPORTED, 21 skills) = **6157 chars — under budget, fine.** This is what Claude actually loads everywhere.
  - `<repo>/.claude/skills` → `.agents/skills` (per-repo, Codex auto-discovers) = the real ceiling surface. Swept all repos: most small (135–1743); **intel = 10002 chars, the one genuine overage** (27 intel-domain investing skills: asset-decision, dcf-model, earnings-preview, talent-dossier…).
- **Fixed the real one:** trimmed intel's 11 fattest descriptions' enumeration/phase-list prose (body-content, not routing) → **9966→7783 chars, under the ceiling with headroom**, every routing trigger preserved (double-quoted-trigger preservation gate + eye-check of single-quoted/slash triggers + full YAML parse). intel@5b449075.
- **Lesson (metaimprove):** the budget-measurement command in `context-budget-principles.md §7` globs `~/.claude/skills/*` (correct population for the CLAUDE index) but the ceiling that actually BINDS is **per-repo Codex `.agents/skills`** — a different surface the §7 command doesn't sweep. The `[obs]` author ran the command against the source dir and got a number that's neither. **No new lint** (measure-before-enforce, and it's now under): if per-repo Codex indexes re-bind, the check is `for d in ~/Projects/*/.agents/skills; do …` not the global glob.
- **Status:** [x] real overage fixed (intel); [obs] corrected (global index was never over budget — the finding was a wrong-population artifact).

### [2026-06-14] Cross-repo context-budget sweep — always-loaded thinning + the standing canary
- **Trigger:** Markus surfaced context bloat across repos ("genomics for example", "lots of bad context running around", `#f` on MCP scoping). VOI-sequenced (see `decisions/2026-06-15-voi-sequenced-review.md`): scout all repos → fan out 4 read-only audits → vet → apply the safe high-VOI cuts → wire a standing canary.
- **Shipped (all reversible, verified via `context-budget.py`):**
  - **scite MCP → project-scoped** (user `#f` lever): moved scite (bio-only citation MCP, ~11.5k deferred-tool surface) out of `~/.claude.json` user scope → genomics+phenome `.mcp.json`. Drops ~11.5k from EVERY non-bio session (agent-infra/intel/hutter/…) next-session. Backup at `~/.claude.json.bak-scite-move`. Search MCPs (brave/exa/perplexity/research) left user-scoped (general); research-MCP-consolidation deferred (prior art `search-tool-thinning-*.md`).
  - **Standing budget canary** (the RSI convert — 2nd context-budget loop-miss this session after skills-index): `context-budget.py --check` flags any active repo over a 30k always-loaded ceiling; wired report-only into drift-sentinel → SessionStart digest. Catches drift the loop previously missed (genomics crept 32.9k→39k unwatched until Markus ran /context). agent-infra@e9438bb.
  - **genomics path-scoping**: 11 domain rules always→conditional, 39.2k→29.4k always-loaded. genomics@fa8a3c297. (Worktree-isolated audit → cherry-picked to live + each scope spot-checked.)
  - **phenome path-scoping**: 3 rules scoped, −1,981 tok. phenome@75145ce5.
  - **intel**: NO CHANGE — already optimized (40/44 rules scoped, 4 are cross-cutting KEEP-guards; codebase-map glob already `tools/**,analysis/**`). Audit was stale.
- **Lesson (intern-rule + prior-context, both dogfooded):** subagent/audit TOKEN figures were untrustworthy — the genomics audit reported **chars as "tok"** (~4× inflation: modal-dx "9.4k"=2,335 tok); the intel+modal-dx audits were **stale on STATE** (work already done). Markus's prior-context flags ("we did modal-dx→modal yesterday", "we discussed MCP before") + spot-checking every load-bearing number against `context-budget.py`/git caught both. **Rule reinforced: trust the deterministic measure + git, never a subagent's asserted numbers.** Also caught: worktree isolation branches a CROSS-repo subagent off stale `origin/main` ([[worktree-isolation-branches-origin-main]]) → cross-repo apply-subagents must edit the live checkout directly, not a worktree.
- **Status:** [x] always-loaded thinning shipped (scite + genomics + phenome + canary; intel already-clean). [ ] OPEN — the dominant remaining genomics chunk is **codebase-map.md (~34.7k, path-scoped to scripts/**)**: not always-loaded, but it's most of the ~80k a genomics code-session loads. Fix = HIERARCHICAL map (top-level tree + per-subtree maps with tight globs) via the generator (`scripts/codebase-map.py` + `refresh-codebase-maps.sh`) — a focused build, Markus's "more hierarchical / tree→indexes→maps" idea. [ ] deferred: agent-infra 2-rule scoping (low-value, under ceiling); genomics/intel CLAUDE.md+MEMORY.md trims (~6-8k each, human-owned → review).

### [2026-06-15] [x] agentlogs: index Cursor CLI (`cursor-agent`) sessions
- **Gap:** Cursor Agent CLI transcripts live at `~/.cursor/projects/*/agent-transcripts/*/*.jsonl` but were invisible to `/observe`, blindspot-miner, and `agentlogs recent` (Claude+Codex+Gemini only).
- **Fix:** New `src/agentlogs/adapters/cursor.py` vendor adapter; default index vendor list now includes `cursor`. Parses user/assistant messages + inline `tool_use` blocks (tool results not exported by Cursor yet → completeness=partial).
- **Verified:** 305 sources discovered; index + `agentlogs recent --vendor cursor` shows this session.
- **Status:** [x] implemented

- **Evidence:** test-health 2026-06-15 `0→6` failures then `timeout` at 900s (713s when completing). Root causes: `gate_fixture.py` missing entity-resolution DDL on lagging live DB; `kg_pack_egress_inventory.py` ADR 0023 grounding tables not in `SHIPPABLE_TABLES`; suite norm ~12–13m flirts with 900s ceiling under load.
- **Fix:** entity-resolution patch before views.sql in gate fixture; add admissibility_gate_requirements/grounding_policy_epoch/grounding_verdict/current_grounded_support to SHIPPABLE_TABLES; phenome sentinel timeout 900→1200s in `scripts/test_health.py`.
- **Verified:** 4 targeted phenome tests pass; full run 1247 passed.
- **Status:** [x] implemented

- **Run:** /observe sessions+failures+blindspot+drift+supervision (session 9547ab33), then /improve maintain.
- **Sessions:** CLEAN — 9/10 Gemini quality=1.00, one 0.97 (Codex bare-python block fired + self-corrected). Zero mandatory failures. No anti-pattern recurred ≥2 sessions.
- **Failures (21d, 84 clusters):** dominant class = bare `python3`→ModuleNotFoundError (duckdb×46, requests×30, edgar×30…). **Already fixed THIS session** by `pretool-uv-python-guard.py` (skills@e930dc4→649fbc3, built 14:41); all top clusters' last_seen ≤06-13 = pre-hook backlog. CLAUDE.md "hook blocks bare python" is now ACCURATE. Net-new actionable: 0.
- **Blindspot (91 misses, 7d):** #1 cluster = prior-context/doc-currency ("check the git logs", "did you update docs"). **Already converted** by `userprompt-prior-context.py` (skills@a174cf0 / ~/.claude@ee66cd6, 14:41, pre-reg metric ~06-28). All 91 misses dated 06-08→06-13 = pre-hook backlog. Covers blindspot #1 AND #2 (doc-currency keywords in trigger). Net-new detector: 0.
- **Supervision (7d, deterministic):** 0.9% wasted (35/3868), far below 15% target. Clean.
- **Inventory-guard win:** the loop rediscovered two completed-this-session fixes and correctly closed them as backlog (inventory-before-dispatch working as designed).
- **Net actionable:** ONE — drift recurrence pair (SSH-burst fail2ban + pgrep self-match, 2 sessions each, transcript-verified, ungoverned) → escalated to `decisions-pending/2026-06-14-remote-ssh-ops-gotchas.md` (global-rule home = human-owned). **[x] RESOLVED 2026-06-14:** Markus approved A → `~/.claude/rules/remote-ssh-ops.md` shipped (~/.claude@466a277); pending memo removed.

- **[obs] Probe-before-dispatch miss at WORKFLOW scope, 2026-06-14:** launched a 5-agent genomics context-trim workflow (~331k subagent tok) off the SessionStart drift-digest's stale `genomics 38941 OVER` without re-running the live canary FIRST. Live `context-budget.py --check` = `29580 ok` — a peer had already path-scoped 11 rules (genomics@fa8a3c297, ~39k→29k) hours earlier. Caught at the EXECUTION boundary (probe-primitive-first) before 14 redundant/conflicting edits on a peer-active repo — but the analysis tokens were already burned. Lesson: re-verify the TRIGGERING metric live (the actual canary command, not the daily-stale digest proxy) BEFORE dispatching a fix-workflow; inventory-before-dispatch applies at workflow-launch, not just subagent-research-launch. Covered by `probe-primitive-first.md` + inventory-before-dispatch — a non-application, not a new rule. Residual win salvaged: 2 stale CLAUDE.md facts fixed (genomics@906ade7d5).

### [2026-06-15] /research skill routing — index visibility + compliance gate (RSI close)
- **[obs] SKILL-INDEX VISIBILITY (verified):** agentlogs audit found `/research` typed in ~163 sessions/60d but Skill invoked only 29% (noise-filtered). Root cause split: (1) agent skips Skill for ad-hoc search; (2) `research` description sat at cumulative char **7912** in the global skills index — past Codex's **8000** char ceiling, so the model barely saw it. `uv run python3` probe after description rewrite: `research prev_cum=7497 line_len=256 fully_visible=True` (`just skills-budget` still 10088/8000 total — tail skills clipped). Shipped `pretool-research-skill-gate.py` (science-scoped `/research` → block search until Skill(research)); `python3 test_pretool_research_skill_gate.py` → **5/5 OK**; `skill-routing.py --cases` → PASS. Description ads now lead with `/research` + "INVOKE BEFORE ad-hoc search". Residual: trim ~2k chars from index tail (`verify-before`, `writing-style`, …) to get fully under budget.

### [2026-06-16] `just` dispatch unification — Phase 1 shipped (`just gather` + `just critique`)
- **[x] SHIPPED (main be47a59/57bb630/4191acc):** Deterministic v0 gather (`scripts/gather_context.py`, no LLM — extracts referenced paths, existence-checks, honest `missing:`) → `just critique` fronts the existing `model-review` engine (genomics `plan-close-review` pattern), collapsing the orchestrator's ~4-call packet-assembly into one. Brief-schema contract: `.claude/rules/dispatch-brief-schema.md`. Verified end-to-end: gather → Composer premise-scout → 4 axes → 11 extracted findings on the loop-core ADR. Phase 1 of `.claude/plans/2026-06-16-just-cli-dispatch-unification.md`; the 5-run measurement gate is deferred (structural turn-saving evident, quantification ≈$2).
- **[obs] STALE-MANIFEST AUTO-LOAD trap (recurred 2 sessions: 06-15, 06-16) — NOT pre/stop-hookable:** `model-review.py:2909` auto-loads `.model-review/dispatch.json` with no staleness check, so a manifest from any prior `review_gate triage` (this session or a peer) silently poisons the next dispatch with its blockers, and `_scan_dead_refs` hard-blocks design docs that legitimately name cross-repo/basename refs. The failure lives inside a subprocess's file-loading logic, not at an agent tool-call boundary a PreToolUse/Stop hook can intercept → resists hookification. Architectural fix is in-script (mode-gate the blocker to `close`; staleness-gate the auto-load) and is **shared skill (3+ consumers) → proposed in `ideas.md`, needs sign-off**. Recipe-local mitigation already shipped (`rm -f dispatch.json` + no triage step). (critique-to-hooks-protocol determination.)
...
- **[x] observe size-safe context:** `scripts/observe_prepare_context.py` + `just observe-context` (580KB cap); **2026-06-28:** `--days` forwarded to extract scripts (default 1).
...
- [obs] [2026-06-20] DRY-RUN SIDE-EFFECT CLASS: `--dry-run`/preview modes mutated the TARGET repo — debug_scout wrote a stub, debug_until_dry wrote a memo then an empty `docs/audit/` dir; 3× this session + debug_scout `4489b3b` historically. The first instance (a stray dry-run into `substrate/docs/audit/`) opened the whole session. Now fixed in both live scripts (dry-run touches nothing) + gitignore `**/`-anchored. Promotion gate: a side-effect-free-`--dry-run` invariant test for the orchestrator-script family ONLY on a 3rd such script — building it for 2 is premature, and a hardcoded-2 test would re-create the silent-cap false-confidence trap (the very class `[:6]` caused this session). If built: auto-discover scripts exposing `--dry-run`, don't enumerate.

### [2026-06-20] Phase -1 — improvement-log bulk triage (RSI unification)
- **Grandfathered:** prose-only `[ ]` / `[obs]` rows predating structured frontmatter (lint ships Phase 1).
- **Open `[ ]` count:** 1 — output truncation on long Opus sessions → deferred architectural item; not tier-0 motor until eval path exists.
- **agentlogs-index:** doctor reports all vendors fresh (2026-06-20); IndexerLockBusy exits 0 by design when another indexer holds the lock.

- [obs] [2026-06-24] UV-GUARD RESIDUAL = INVOCATION NOISE, NOT MISSING-DEP (don't re-chase top_priorities #1). The "Recurring missing-dep failures across N modules" priority is dominated by `duckdb` (18 fails/8d) — but `uv run python3 -c "import duckdb"` → OK 1.5.2 (declared+installed agent-infra dep); only bare `python3` fails. The whole 64-cluster set is bare-python / unsynced-ephemeral-worktree invocation, NOT a project lacking a dep — the action hint "add deps where a real project lacks them" has **zero targets**. `pretool-uv-python-guard.py` selftest PASSES and auto-rewrites bare→`uv run` in uv projects (since 2026-06-21); residual leaks come from contexts the PreToolUse Bash hook can't reach: **cursor/Composer sessions** (not gated; agentlogs has claude+codex+cursor) and subprocess/`just`-recipe bare-python. Long tail (`pipeline_stages`, `arc_agi`, `click_solver`, `witness_grid`… 1-fail/1-day) = pure ad-hoc wrong-cwd local-module noise. **Fix is NOT dep-add** — it's extending bare-python coverage to cursor + subprocess invocation (real work → plan candidate, not inline), OR down-weighting single-fail local-module rows in `top_priorities.broken_tools()` so #1 stops over-stating. Promotion gate: only build the cross-vendor extension if a Composer session's missing-dep failure causes a real downstream loss (not just a logged ModuleNotFound the agent self-recovered from).

- [obs] [2026-06-24] CONSOLIDATION LEFT 4 ORPHAN LAUNCHD PLISTS (exit 127, daily). `d4c553a` ("consolidate 7 jobs → pulse-tick, .sh→.py") deleted the wrapper `.sh`s but left the old plists loaded — `blindspot-miner` + `drift-sentinel` + `act-drain` + `maintain-tick` all failed exit 127 daily on missing scripts; their functions are live in `pulse_tick.py` (drift L63, drain L141, motor L177, blindspot L92). All 4 removed this session (a1f7e34 + tick-1). `gov-report` was in the docstring's "replaced" list but its `.sh` EXISTS + exits 0 + is consumed daily (`artifacts/gov/gov-report.md`) → KEPT. Class lesson: a consolidation commit that deletes wrapper scripts must `launchctl unload` + `rm` the orphaned plists in the SAME commit; a SWEEP-time check (`launchctl list | grep agent-infra | awk '$2==127'`) catches the residue but only after it leaks failures. Candidate guard: post-consolidation lint that asserts every loaded `com.agent-infra.*` plist points to an existing ProgramArguments path.

### [2026-06-24] Trending scout findings (window 06-19→06-24) — routed actionable items
- [x] CC 2.1.186 `Agent(type)` permission rules now ENFORCED for named subagent spawns — audit `settings.json` (global + project) for any `Agent(...)` allow/deny rule that now silently blocks an intended dispatch lane (we dispatch named subagents heavily). Source: research/trending-scout-2026-06-24.md #1. **[x] 2026-07-04 drain:** audit run — zero `Agent(` typed rules across global + all 32 project settings files (only bare `"Agent"` allows and hook matchers, which can't block a named lane); nothing to fix.
- [ ] CC 2.1.187 `sandbox.credentials` — enable to block sandboxed commands from reading credential files + secret env vars in autonomous/sandbox-mode runs (cheap security harden). Source: trending-scout-2026-06-24 #2. **Revalidated 2026-07-04 (drain):** unset everywhere; NOT a config one-liner — llmx and dispatch scouts read keys from `~/.env` inside sandboxed Bash, so enabling could break those lanes; needs a deliberate policy call, keep open.
- [-] MCPProxy `retrieve_tools` lazy tool-loading — probe the on-demand-tool-search pattern against our always-loaded `.mcp.json` context budget (every subagent spawn pays ~16.7K always-loaded tok); measure context saved before deciding. Source: trending-scout-2026-06-24 #3, extract-pattern. **[-] 2026-07-04 drain:** moot — CC native deferred-tool loading (ToolSearch/MCPSearch, v2.1.7, default-on above 10% context; research/claude-code-internals.md:339) already delivers exactly this; confirmed live in-session (all MCP tools surface deferred). Item was moot when filed.
- [x] Codex rust-v0.142.0 subagent terminal-error propagation (#28375) — re-check whether the subagent-zero-output gate (claude-code#47936) needs adjustment now that Codex parents RECEIVE terminal subagent errors instead of empty successful completions. Source: trending-scout-2026-06-24 #5, evaluate. **[x] 2026-07-04 drain:** re-check done, no adjustment — the gate's rationale is claude-code#47936 (web-verified still open) plus stub-first output durability, which Codex's detection-side fix doesn't touch; removal trigger in memory `subagent-zero-output-gate-stays.md` unmet (zero-output rate 4.2% vs <0.5% bar).
- RECONCILIATION: trending-scout-2026-06-24 — remaining findings are study-only (read-deep papers 2605.30621 / 2606.15363 / 2606.06324; MiMo-Code `/goal` judge-gate extract-pattern; respondToBashCommands watch; version bumps) → memo-only per routing protocol; not inflating the `[ ]` queue.

- [obs] [2026-06-24] OVER_CAUTION ENFORCE-GRADUATION EFFICACY = CONFOUNDED, not demonstrated (F3 falsify gate, run manually). Blindspot digest shows over_caution still 50/7d despite the detector (`stop-smart-judge.sh` vector #3) being graduated shadow→enforce ~06-16. Dedup confirmed: detector EXISTS + fires cross-vendor (`smart-judge:15`/24h in a Codex session's hook-trigger line) — NOT a coverage gap, do NOT rebuild. Measured the behavioral claim ("reduces over-ask"): over_caution fell 5.0→1.44/day (−71%) pre vs post 06-16 — BUT every control class fell too (error_correction −66%, rediscovery −69%, repeated_instruction −52%; similar session volumes 533 vs 509). So the drop is a **system-wide correction decline**, NOT attributable to the detector → CONFOUNDED per `predict-then-falsify-gate.md`. Without the control this reads as a false "71% win." **Lesson: never claim a supervision-detector worked from the target metric alone — the control is load-bearing.** Real infra gap (not built — manual script sufficed this run): over_caution has no queryable agentlogs signal (mined ad-hoc via supervision-kpi), so the F3 gate can't run formally with a control — materializing supervision corrections into agentlogs would let graduations be auto-falsify-gated. Promotion gate: build the materialization only if a 2nd supervision-detector graduation needs falsifying (2+ bar).

- [obs] [2026-06-24] RSI close — `over_caution` detector is now a GOV-SHRINK ABLATION CANDIDATE (the strictly-better disposition of the CONFOUNDED finding above; not eradicating on faith). It has MEASURED cost (27–38s latency on flagged turns + metered haiku-api fast-path against subscription-default policy, already flagged) and CONFOUNDED benefit (can't isolate it from system-wide correction decline). Per gov-id telos (a scaffold earns its place only while the model fails the verifier WITHOUT it; governance shrinks as IQ rises), the correct next step is `maintain_tick.py --subtract --ablate` on `hook:over_caution` — run it removed, measure if over_caution rises vs the control; retire if flat. Do NOT keep a costly detector on faith just because its target metric moved with everything else. This is the recursive-self-improvement move the session's falsify-gate earned: the loop should ablate its own graduated scaffolds, not only add them. Session meta: 12 commits, all instrument-layer paydown (canary ×3 robustness, indexer size-skip deadline fix, top_priorities truth, launchd orphan-detector, falsify-gate) — the instruments are now trustworthy, so future ticks should hit fewer false fires and reach real cross-project work faster (the leverage a future agent inherits).

### [2026-06-24] RESOLVED: risky-diff-review SHADOW — CUT (not promoted); correlation half FALSIFIED by negative control
- **Disposition of** the `[2026-06-07] SHADOW: risky-diff-review demand probe` finding (its self-imposed "Promote/cut ~2026-06-21" date passed; a self-imposed date is a reminder that fires only when an agent acts, so this tick acts). Gov-ID `hook:risky-diff-review-shadow`.
- **Criterion was:** promote to an auto-review trigger ONLY if it (a) fires often AND (b) unreviewed-risky commits correlate with later fixes; cut if rare. Both halves measured against the full 192-row shadow log (window 2026-05-08 → 2026-06-20, 6wk):
  - (a) fires-often: 19 review-worthy over 6wk ≈ 3/wk — modest, not a flood. Borderline at best.
  - (b) correlate-with-later-fixes: **FALSIFIED by negative control.** 9/19 (47%) review-worthy commits had a later fix/revert/repair touching their files — BUT a control sample of 30 NON-flagged (reviewed/tested) commits to the SAME high-churn dirs (skills/hooks, scripts, .claude/rules) hit the IDENTICAL **14/30 = 47%**. The flag has ZERO discriminative power; the 47% is pure file-churn baseline (hooks/infra churn → later "fix" commits regardless of review). Same confound class as the over_caution finding above — an uncontrolled correlation that tracks a baseline is CONFOUNDED, not FIRED (`predict-then-falsify-gate.md`).
- **Plus** the original premise (self-review degeneracy) was already disconfirmed on Opus 4.8 (`decisions/2026-06-03-verifier-bound-autonomy.md`) — the live hypothesis going in was CUT.
- **Verdict: CUT — do not build the auto-review gate.** The shadow did its job: it answered the binary "build an auto-review gate, y/n?" with a controlled NO. Its named consumer (that decision) is now resolved → the detector is generation-without-consumption going forward.
- **Status:** [>] decided — NOT promoted (premise disconfirmed + correlation falsified by negative control). The detector-infra eradication (gov-shrink: remove `risky_diff_review_shadow.py` + the 3 just recipes + the gov.py wiring) is governance DELETION → routed to `decisions-pending/2026-06-24-retire-risky-diff-review-shadow.md` for human sign-off (never auto-remove governance in an unattended tick).
- **Lesson (reinforces over_caution above):** the negative control is load-bearing for ANY "this detector/flag predicts X" claim. Two confounded-correlation findings in one session (over_caution efficacy; risky-diff→later-fix) — both would have read as real wins (−71%, 47%) without the control. The control is the cheapest possible falsifier and the loop should run it before promoting any shadow.

- [~] [2026-06-24] RETIRED — EXECUTED: risky-diff-review shadow eradicated (operator sign-off "if you have conviction ... do it"). Deleted `risky_diff_review_shadow.py` + oneshot.sh + test + `just risky-diff-shadow`/`risky-diff-report` + the `unreviewed_risky` block in `gov.py` + the `com.agent-infra.risky-diff-review` plist (booted out). 53KB evidence log archived to `/Volumes/2TBPNY/claude-archive/shadow-logs/` (not destroyed). Verified: gov report renders 0 risky refs (exit 0), 25 gov tests pass, hooks-smoke 274, feature_loop_probe (kept sibling) intact. ADR: `decisions/2026-06-24-risky-diff-review-shadow-retired.md`. Gov-ID `hook:risky-diff-review-shadow` closed.

- [obs] [2026-06-24] TOKEN WASTE / POLL-LOOP ON A BUFFERED BACKGROUND JOB (observe sessions incremental, personal 44cc879b, 1x novel). Agent launched `gallery-dl ... -j` (buffers ALL stdout to the end), then spun 12+ turns of `until`-loops + status re-greps on a job that structurally never streams ("ENUM STILL RUNNING", 0 bytes >10 min), only switching to a streaming `-g` flag at ~turn 18. The non-streaming behavior of `-j` is knowable at the first stall. **Class:** waiting on a non-streaming job — adjacent to `posttool-bash-poll` dup-stat guard, but the wait primitive here is the harness-sanctioned `until`-loop (not bare repeated stats), so that guard never fires. **NOT cleanly hookable** (critique-to-hooks step 5): `until`-loops are the correct wait primitive and "buffered-never-streams vs legitimately-slow" is a semantic call, not a checkable predicate. Disposition: `[obs]` only; watch for a 2nd instance before any rule. POSITIVE calibration datapoints same window (not promotable): agent-infra 70c686a8 /improve->/decide arc held spine + collapsed impl ~60% via cross-model verify-before-fold; 78a59222 hit Pre-Build #1 (extended `worktree-gc`, did NOT build a duplicate recipe) + correct autonomous-vs-shared blast-radius split. Evidence: artifacts/observe/{candidates.jsonl,digest.md} (gitignored); parent-direct analysis, no Flash dispatch.

- [obs] [2026-06-24] agentlogs-index exit 75 every run = the HARD SIGALRM backstop (max_run_s+60), NOT the graceful soft-defer. Investigated (probe-primitive-first): plist is StartInterval=7200, NO KeepAlive → exit 75 drives no launchd retry (purely informational). Index stays FRESH (last_index_success current for all 3 vendors, parse_errors_7d=0); orphaned_7d already tracks deadline-truncated runs independent of exit code. Size-skip fix IS deployed (deployed index.py == source). So exit 75 is benign — the deferral wrap-up overruns budget+60s. Did NOT change exit 75→0: that would MASK a genuinely-firing backstop (epistemic #8 fail-loud, unlike pulse-tick's unambiguous advisory-drift bug). WATCH: if last_index_success ever lags, the budget is too small for the corpus → tune max_run_seconds. Benign now.

### [2026-06-24] F3 predict-then-falsify gate is CONSUMER-TIER-BLIND (inward sweep finding)
- [-] **Status:** REJECTED-as-premature (validated 1/10, 2026-06-24 — the finding's own validate-first gate ran). The F3 gate has **0 real mechanism-records** (only `mechanism-records/example-tool-error-rate.json`, a template), so refining its tier-segmentation is Pre-Build #3 (dead code with a plan attached). Tier-spread IS real (codex/claude/cursor + Opus/Haiku subagents) but moot until the gate is adopted. Prior gap = gate ADOPTION (this session ran 2 F3-style analyses MANUALLY, bypassing `mechanism_record.py`; already tracked via the over_caution `[obs]` agentlogs-materialization item, 2+ bar). **Resurrection:** revisit only once ≥3 real mechanism-records span tiers. Original rationale kept ↓. The F3 gate (`scripts/mechanism_record.py`, `predict-then-falsify-gate.md`) aggregates `metric_query`/`control_query` over ALL sessions — it never segments by the consumer model the changed rule/hook runs under. Per Harness-Updating-Is-Not-Harness-Benefit (2605.30621, METHOD not rate): a harness change's BENEFIT is non-monotonic in consumer strength, so a governance change validated on the operator's Opus turns can be neutral-or-harmful for a Haiku/cheap subagent that also reads the rule. The gate scores it FIRED on aggregate → we keep it, blind to the weaker-consumer regression. This is the held-out/per-consumer discipline the frontier formalizes, applied to our own gate (which BIT twice this session — over_caution + risky-diff both CONFOUNDED on the control half).
- **Proposed fix:** add a consumer-tier split to the metric/control queries where `runs.model` varies (segment FIRED/CONFOUNDED per consumer tier, not just aggregate).
- **Validate at 1/10 FIRST:** measure how many governance changes even HAVE cross-tier consumers before building the split — binds only where a rule's consumers span tiers AND the per-consumer signal exists. Many rules are operator-only (exempt).
- **Source:** `research/2026-06-24-rsi-loop-vs-harness-evolution-sweep.md` (inward sweep, our loop vs the 06-24 harness-self-evolution frontier).

- [x] [2026-06-24] HARNESS-EVAL FLAKY STEP: `system_inventory.py --check` — **fixed 2026-06-28**: `_normalize_volatile_inventory()` strips live launchctl lines from `--render --check` compare; harness-eval now calls `--render --check` (bare `--check` was a no-op). Real drift still via `orient --drift`.

### [2026-06-25] COST / SILENT-SPEND: backgrounded-pipeline metered spend is invisible to the cost-guard ledger
- [obs] [2026-06-25] **METERED-ESCALATION DEFAULT COST $70 (mechanism FIXED in-repo; cross-cutting backstop is the residual).** substrate 018ee7c8 @ 2026-06-22T21:17, operator: *"NEVER DO THAT!!! I SAID CODEX 10000 TIMES... costs me $70."* `corpus_extract.extract` had an escalation fallback (`escalate_subscription=False`) that silently crossed into the **metered API** when a codex `$0` call yielded 0/timed out, accumulating $70 across a bulk run. Operator had restated the codex-`$0`-subscription default **≥8×** across the multi-day session (06-17T14:46…06-22T14:37) — the routing default kept drifting back to the expensive lane (the behavioral shadow). **Mechanism fully resolved in substrate**: `escalate_subscription: bool = True` is now the default AND `_build_cmd` *hard-refuses* metered GPT in extraction (substrate@6dd4f84 "Cost firewall", @c8af5ee docstrings). The `escalate_subscription` metered lane recurs in 2 substrate sessions (018ee7c8, 67fbcd90); the broader COST-OVERSIGHT class also bit genomics 1a19b57e ("budget ran out... runs looped... you forgot to check" — distinct modal-spend-loop mechanism, same family). Recorded `[obs]` so the mechanism isn't re-chased.
- [>] [2026-06-25] **BACKSTOP GAP (the promotable residual): cost-guard cannot see spend outside foreground Bash.** `pretool-cost-guard.sh` (and raw-openai-guard/companion-remind) gate **interactively-typed** `llmx`/provider Bash commands against the $25 daily cap — but the $70 spend lived inside a **backgrounded Python worker's escalation branch**, structurally invisible to a PreToolUse Bash hook. The class fix (independent of any one pipeline's per-call default): route backgrounded/pipeline metered `llmx` calls through the **same daily-spend ledger** the cost-guard reads, so escalation spend trips the cap regardless of invocation surface. This *extends the existing [obs] at improvement-log:3601* (stamp caller/skill ID into `llmx-usage.jsonl`) from attribution → enforcement: once per-call metered spend is ledgered, a PreToolUse-or-periodic check can block/alarm when the running total crosses the cap. Agent-infra-local + cross-repo (any pipeline that can escalate to metered). Checkable: simulate a metered escalation in a bg worker, assert the daily-spend ledger increments and the guard fires. NOT built inline — needs the ledger-write path designed (where pipelines emit usage) + one-task-per-tick discipline. **[>] 2026-07-04 drain:** split-consumed — observability half SHIPPED (`llmx-usage.jsonl` was already surface-agnostic; `usage-check.py --metered-today` + `doctor.check_metered_spend`, 27ff16e); enforcement half lives in `decisions-pending/2026-06-25-metered-spend-funnel-enforcement.md`.
- **Root cause:** system-design — cost enforcement is scoped to the interactive Bash surface; agentic spend increasingly happens in backgrounded workers/subagents whose metered calls never pass through a guarded foreground command.
- **Evidence:** substrate@6dd4f84,c8af5ee (in-repo fix); substrate transcript 018ee7c8 21:17/21:19; `pretool-cost-guard.sh`; improvement-log:3601 (attribution [obs] this extends).

### [2026-06-25] SHIPPED (observability half of the $70 BACKSTOP GAP above): metered spend now surfaces across all surfaces
- [x] [2026-06-25] **The funnel ledger already existed — premise of the BACKSTOP GAP entry was stale.** VOI probe: `~/.claude/llmx-usage.jsonl` is appended by llmx for *every* call regardless of surface (foreground Bash, backgrounded worker, OR Python subprocess) and already carries `transport: "api"` (metered) vs subscription, plus `caller`/`cwd`/tokens. So "the ledger-write path needs designing" was wrong — the surface-agnostic funnel was built; only a *consumer* was missing. Shipped `usage-check.py --metered-today [--alarm N]` (filters `transport==api`, today, priced, grouped by `(caller, repo)`, exit-1 on alarm; also fixes the latent over-count where the default report priced subscription rows as metered) + `doctor.check_metered_spend()` wires it as a **daily consumer** (warn $10 / fail $25 constitutional cap). A background metered runaway like substrate's $70 now surfaces in `doctor.py` the same day instead of at billing. NOT dead infra — doctor runs daily. Commit this session.
- [>] [2026-06-25] **HARD-BLOCK enforcement half → escalated** `decisions-pending/2026-06-25-metered-spend-funnel-enforcement.md` (shared, invariants #4): the residual is *blocking* (not just alarming) at the funnel — in-llmx pre-dispatch guard (principled, surface-agnostic) vs periodic launchd alarm vs extending `pretool-cost-guard.sh`; plus the $25-vs-$1000 cap discrepancy (constitution $25 / hook blocks at $1000) and the unpriced-model undercount (`RATES` has `gemini-3-flash` but live `gemini-3.5-flash` priced $0 — an alarm that undercounts misses real spend). Touches shared hooks / llmx (3+ projects) → human sign-off. **[>] 2026-07-04 drain:** duplicate surface — the canonical ask is the decisions-pending file (verified still pending, surfaced in `just questions`); tracking it here too double-counts the open queue.

### [2026-07-04] RSI close — goal-night controller live-fire (this session)
- [obs] [2026-07-04] **UNVALIDATED-LAUNCHER-ARG armed a goal-run on the authoring session.** `just goal-night --help` (typed as a help probe during testing) bound `--help` to the `ritual` positional → wrote it into `.claude/goal-run` → unparsable content fell back to the 100K default → the Stop-hook ritual fired on the AUTHORING interactive session at ctx=193,444. Silver lining: end-to-end live proof (threshold math, ritual injection, once-only latch all correct). Class: a just recipe with positional defaults happily eats flag-looking args — validate token-count args in any recipe that ARMS behavior (fixed for goal-night in 46b9a3e: integer check + 80K floor + ritual<window). Verified claims this close: native auto-compact fired at configured window (`trigger:"auto",preTokens:87707`, probe 6510cb77); controller fix landed (46b9a3e).

### [2026-07-05] HARVEST: harness-requote class — 3 signal miners counted compaction/meta re-quotes as fresh user signal
- [x] [2026-07-05] **#f/#g extractor silent false-zero + the shared harness-requote class, fixed at all 3 in-scope consumers.** (1) `skills/improve/scripts/extract_user_tags.py` read `msg.get("content", msg.get("message"))` — the fallback returned the message **dict** on the modern envelope format, so text="" and the harvest's Phase 3a "highest-signal" source reported 0 forever; also `--tag` (passed by SKILL.md) was unimplemented (`--tag g` silently returned #f). Fixed + 7-test pin (skills@837f4d2); post-fix window yield 3 #f + 3 #g, all real. (2) Same-class sweep (rule #21/critique-to-hooks step 2): `reflect_capture.parse_events` mined #f re-quoted inside compaction summaries → the 2026-07-04 quarantine MINTs whose evidence was 5× the compaction preamble at conf ≤0.06 (agent-infra@c0c2f22 + regression test). (3) `blindspot_miner._user_text` SKIP regex missed `isCompactSummary`/`isMeta` → duplicate flags in the digest (agent-infra@fd3209a). Residual out of scope: `export_sessions_for_emb.py` reads the agentlogs DB (different plane — whether the indexer stores summaries as events is an agentlogs question, not a transcript-parse bug). Class rule for future miners: any raw-transcript signal miner must skip `isCompactSummary`/`isMeta` lines.

### [2026-07-05] HARVEST: bash-command-parsing hooks re-implement strip/tokenize separately — proven-common at ≥3 consumers
- [ ] [2026-07-05] **Extract one `bash_cmd_strip` helper (heredoc-body strip + quoted-string strip + command-position anchor) in `skills/hooks/` + drift-test; migrate no-background-commit, bash-loop-guard, commit-exit-masking.** 4 false-positive/edge-case fixes in 3 days (skills@ff79b2d heredoc, @7af4faa quoted-string, @681a068 pipe-masking, @6f867eb) — each hook drifts its own copy, and a divergent stripper is a *correctness* failure (false block or silent pass), which passes both the proven-common bar (≥3 consumers) and epistemic #9 (one definition, consumers load it). **Propose-only** (shared hooks, 3+ projects — invariants #4): needs human sign-off before deploy.

### [2026-07-05] SHIPPED (root fix for the harness-requote class): agentlogs labels harness-injected lines at ingest
- [x] [2026-07-05] **The class is now a schema property, not a per-miner discipline.** Claude adapter labels `vendor_kind = compact_summary|meta_injected` on user_message events (label-never-drop — ledger stays forensic; f135857); consumers filter: emb export (f135857), sessions.first_message + skill-routing prompt attribution (5de6416). Raw-JSONL miners single-sourced onto `scripts/common/transcript_text.py` (a13a0de) with a drift-test pinning the skills-repo vendored copy. Historical backfill of compact-summary rows by text prefix ran under the indexer flock; `meta_injected` backfill skipped (no text marker — rolls out via 21d retention). /improve skill: harvest read-path pruned to live-streams-first + denominator rule + P3 due-ness derived by `rotation_due.py` (skills@536749b..9881705). Residual: `supervision_session.py` filter deferred (peer session mid-edit); `export_for_selve`/other agentlogs consumers pick the filter up when they next touch these queries.
- [obs] [2026-07-05] CORRECTION to the entry above: the compact-summary backfill was still QUEUED behind the indexer flock when the entry was committed, not yet run — premature-completion claim caught same-session. Verify with: sqlite3 ~/.claude/agentlogs.db "SELECT COUNT(*) FROM events WHERE vendor_kind='compact_summary'" (expect ≥380 once drained).
- [obs] 2026-07-05 arc-agi 0273a01d: a ratchet metric that only PRINTS (no persistence) went dark 5d/118 commits unnoticed — class fix: principal instruments persist append-only by default + a staleness line on the boot surface + close-gates load the ONE canonical threshold (status.py --conversion). Same-day proof: the new debt line fired on the afternoon's own 42-commit run. Details: arc-agi research/2026-07-05 memos + project_conversion_ratchet_debt.md

### [2026-07-06] Maintain tick — test-health red root causes (5 fixed + phenome collection)
- **Source:** `/improve maintain` sweep; `com.agent-infra.test-health` alarm (agent-infra failing 4→5; phenome `did_not_complete` every run for days).
- [x] **test_top_priorities fixtures un-rotted** (`23ae5f9`): hardcoded `last_seen: 2026-07-01` aged past `FAILURE_STALE_DAYS=3` → promote-path test red, suppress-path vacuously green. Fixtures now derive from `now()-1d`.
- [x] **test_agent_maintainability dates from now()** (`7c7c5f7`): `2026-02` commit literals aged out of the `days=120` lookback. Same time-bomb class.
- [x] **test_close_fixes timeout-scaler born-red** (`bfa00b6`): asserted the 1200 cap at 80MB but `120+80*12=1080`; now pins linear region AND cap via sparse-truncated files.
- [x] **approval-tiers manifest coverage** (`9f3ee1b`): `pretool-noext-nongit-guard.sh` + `pretool-timeout-modal-guard.sh` shipped in skills without manifest entries → `test_approval_tiers` red. Added to `invocation_discipline` tier.
- [x] **reclaim-rotate nightly exit-3** (`94a7a04`): 04:10 cron collided with the 2h indexer single-writer lock; added `--wait-seconds 1800` per `agentlogs prune --help`'s own prescription.
- [ ] **Phenome test collection breaks on `phenotype_contract_models`:** `src/phenome/phenotype_bootstrap.py:8` bare-imports it with no sys.path setup; the consumers that DO set the path point at `ROOT.parent/genomics/scripts`, but the module lives in the `genomics-donortail` worktree, not `genomics/scripts` on this box → collection of `test_code_phenotypes.py` + `test_genomics_phenotype_contract_export.py` fails → phenome `did_not_complete` daily. **Operator note (2026-07-06):** the contract module is used live by pipeline stages / agents / Modal (`phenome-check`, gitignored) — NOT dead code, don't delete the imports. **Fix options (phenome-scoped, propose):** (a) conftest bootstrapping GENOMICS_SCRIPTS before collection; (b) reconcile which genomics checkout owns `phenotype_contract_models.py`; (c) test-health `--ignore` the two modules until the cross-repo dep stabilizes. **Root cause:** cross-repo path coupling (phenome↔genomics worktree drift). **Stream:** phenome test infra.
- **Method flags (suggest mode, no candidates — cross-confirmed GPT-5.5):** fixed the dead `session-analyst/scripts/extract_transcript.py` pointer in `improve/SKILL.md` → `observe/scripts/` (skills@97556d4). Gemini `deep_review` dispatch geo-blocked on this box (400 FAILED_PRECONDITION) → routed to GPT-5.5 subscription lane; not yet a standing fix.

### [2026-07-08] F3 predict-then-falsify gate — tier-split PARKED, gate itself is built-but-unadopted
- **Source:** operator-approved 1/10 probe of the 06-24 routed finding ("F3 gate is consumer-tier-blind"); `research/2026-06-24-rsi-loop-vs-harness-evolution-sweep.md`. Prompted by the Lilian-Weng harness-RSI post (Self-Harness's held-in/held-out per-consumer discipline = the same finding).
- [-] [2026-07-08] **Consumer-tier split PARKED (not built).** The finding is structurally real — `scripts/mechanism_record.py:55-57` `metric_query`/`control_query` return one scalar over ALL sessions with zero `model` segmentation, and cross-tier consumers dominate (agent-infra sessions: Haiku **350** > Opus **95**, because subagents run Haiku and each pays the always-loaded rules). Tier signal is available (`sessions.model`, `runs.model_resolved` both populated). BUT the deciding precondition is absent: **`mechanism-records/` holds ONE file — `example-tool-error-rate.json`, dated the build day (Jun 20). Zero real records in 18 days.** Hardening a `metric_query` that has never run is optimizing a zero-caller path (Pre-Build #3). **Resurrection trigger:** the gate accretes ≥1 real record whose metric counts both tiers → add the split then (Haiku-dominance guarantees it'll matter). Supersedes the 06-24 "add consumer-tier split" action.
- [obs] [2026-07-08] **The real finding is the adoption gap, not the segmentation gap.** F3 (steal #1 of the 06-19 SoTA synthesis) is the *preventive* counter to our measured #1 failure mode (accretion / "policy maze") — and it is built + tested (`test_mechanism_record.py`) + documented (`predict-then-falsify-gate.md`) yet **never adopted**: 0 production records / 18d. Same built-then-unused class as the RETIRED `session_quality` gate (this log [2026-06-24]: built 2026-04-07, 0/3850 scored, "calibration that never happened") and the vetoed scored-quality regression gate. **Process lesson:** the 06-24 sweep routed a *refinement* to the gate without checking its *usage* — a finding that proposes refining infra X must first verify X has a live consumer. **Adopt-vs-retire is an open design call flagged to operator** — wire mechanism-records into the governance-change workflow, retire the gate, or leave dormant; reactive controls (`gov-shrink` + `maintain_tick.py --ablate` retirement drafts) partially cover the preventive gap, so low urgency. NOT unilaterally retiring: governance-workflow decision, multiple viable approaches → propose, don't act.

### [2026-07-09] [x] DETECTOR: prior-context REDISCOVERY covers RSI-meta / ask-yourself steers
- **Evidence:** blindspot-digest 2026-07-08 — arc-agi rediscovery cluster ("RSI : why did you not find/figure this out", "ask yourself … meta", "everytime I mention"). prior-context already fired on propose/diagnose + classic rediscovery; meta-steers missed the regex.
- **Fix:** extend `REDISCOVERY` in `skills/hooks/userprompt-prior-context.py`; test 11b in `test_userprompt_prior_context.py`.
- **Dedup:** not a new hook — same UserPromptSubmit surface. over_caution parallel cluster stays separate (already graduated).
- **Status:** [x] implemented 2026-07-09

### [2026-07-09] [>] HOOK +x partial: git mode 100755 for two doctor-flagged skills entrypoints
- **Evidence:** Reinforces 2026-06-13 +x finding. Doctor flagged `pretool-bg-dispatch-footgun.py` + `pretool-askuserquestion-autonomous-warn.py` (FS +x after chmod, git index still 100644). Harvest: `artifacts/harvest/2026-07-09-9547ab33-harvest.md`.
...
### [2026-07-13] Latency audit → hook-fleet dispatcher pattern (3 shipped) + global context re-slim
- **Trigger:** operator: "commits take 20s+ ... cruft accumulated" + "can we cut/unify/simplify?"
- **[x] Global context re-slim:** always-loaded global layer 68K→49.4K chars (CLAUDE.md 40.3K→31.4K, wakeup-cadence 19.4K→9.2K, context-budget-orchestration folded, remote-ssh-ops relocated to genomics+hutter, eval-token-costs compressed). ~6K tok off EVERY repo + every subagent spawn. Baseline table updated (`context-budget-principles.md` §8). ~/.claude@{239cea5,f526d23,99cf0b7,b202467}, agent-infra@9b885e4.
- **[x] Latency audit (subagent, `docs/audit/2026-07-13-latency-audit.md`):** "20s commits" root-caused to hook design (serial process spawns), NOT repo bloat (git ops <300ms everywhere). Offenders: intel 42 Write|Edit gate spawns/edit (~7.5s); genomics pre-commit guard (ps-per-ancestor-level ×3 sites, O(all-/tmp-trackers) glob, 30-min-mtime trust) + unconditional whole-project pyright; global 2 universal hooks' jq storm (~264ms/tool-call).
- **[x] Dispatcher pattern shipped 3×:** intel@13ad8186+7b36168b (42→1, 7.5s→0.2s/edit, 38×; frozen-snapshot oracle + 8/8 parity); skills@0555c57 (2 universal→1 parse-once, 264→101ms/tool-call, 2.6×; 9/9 parity); genomics@c9325e74f (guard: PID-liveness trust via os.kill(ppid,0), dead-tracker reap-on-encounter, ps-snapshot walk; 22/22 tests, kills the dead-session false-block class — hit twice this session). Pattern promoted to CLAUDE.md Hook Design Principles (importlib-load unmodified main(), snapshot fixture + parity pytest, fail-open, fail-fast on exit-2).
- **[~] Audit claim corrected by measurement:** the guard's ps-loop measured only ~25ms/walk (7 levels) under current conditions — the audited ~5s was likely deep-tree/load + the tracker pile; fix kept as correctness + scaling insurance, headline NOT "5s→100ms".
- **[obs] SUBAGENT-REWIRED-LIVE-CONFIG-BEFORE-REPORTING (1 session, candidate):** global-dispatch subagent swapped live ~/.claude/settings.json to its untested dispatcher and went idle without reporting; parent found it by disk inspection. Every tool call ran unverified code for a window (it passed, but by luck of ordering). Fix direction if it recurs: dispatch briefs for hook/config work must stage the settings edit LAST after parent parity sign-off, or a pretool guard on subagent writes to live settings.json.
- **[ ] IN FLIGHT: bash-dispatch subagent** consolidating ~30 global matcher=Bash hooks (mutator-composition semantics are the hard part; partial-consolidation fallback licensed). Parent reviews parity before commit.
- **[x] bash-dispatch landed:** 28 global matcher=Bash gates → 1 dispatcher (skills@ + ~/.claude@f66eeb7). Measured 5.8x: ~1.2s→0.2s per Bash call (~2.5min/150-call session). Mutators (--no-ext-diff, PYTHONUNBUFFERED, uv-python rewrite) emit identical updatedInput JSON; 14/14 parity + 12/12 independent oracle probes (2 probe 'failures' were wrong probe expectations — originals behave identically). Snapshot oracle order-verified vs LIVE pre-edit enumeration (~/.claude git HEAD was 2 gates stale — audit vs git alone would have missed them).
- **[obs] SUBAGENT-REWIRED-LIVE-CONFIG 2nd occurrence (recurs, promote):** bash-dispatch also swapped live settings.json before reporting and idled silently — same class as global-dispatch earlier today. 2 occurrences in one day → per critique-to-hooks protocol this now warrants the structural fix: brief template for hook/config dispatches must mandate 'settings edit LAST, after parent sign-off', and/or a pretool guard blocking subagent Writes to live settings.json paths.
- **[obs] RSI close d9f6b788 verified (fabrication-risk flag cleared):** the 2026-07-08 session's load-bearing claims re-verified true on 2026-07-13 — `mechanism-records/` still holds ONLY the Jun-20 example (25 days, zero real records; resurrection trigger unfired) and `mechanism_record.py` still has zero model segmentation (`grep -c model` = 0). The unsupported_completion flag was a marker miss, not fabrication.
- **[ ] agentlogs.db size floor is INGEST-side, not retention-side (2026-07-14):** live DB 10.5GB holds only ~21d of data (~500MB/day; sessions min = 2026-06-23, only 110 legacy orphan runs older). Retention prune at 30d correctly deletes ~0 — the 2026-07-05 "prune reclaimed 11MB" was not a bug. The size levers are ingest-side: codex-adapter `message` events = 2.6GB text (49KB avg — whole-context rows, the cf9651f codex whence-blind gap; genuine `user` text is 39MB), `payload_json` = 1.8GB. Candidate fixes when the 10-15GB steady state starts to hurt: label+truncate codex injected-context rows at ingest (raw JSONL on 2TBPNY stays source of truth — state-externalization), drop write-only payload_json kinds (compact.py precedent).
- **[x] Ingest half of the size-floor finding SHIPPED same day (2026-07-14, b9510b1; supersedes the "when steady state hurts" deferral above — operator un-deferred):** `textcap.py` = ONE cap definition (head 4KB + tail 3KB, sentinel-idempotent), loaded by both the codex adapter at ingest and the `agentlogs trim` backfill; `test_ingest_and_backfill_agree` pins the equivalence. Applied live: 74,955 rows capped, text 3,103→518MB, DB 9,715→6,932MB, WAL 0B after checkpoint, FTS==LIKE count (74,961) post-trim. Assistant OUTPUT exempt by design — 266MB vs 3,101MB injected-context; work product stays verbatim. Companion fix f76f93b: `truncate_wal()` after prune/compact/trim — a ballooned -wal (observed 4.0GB holding 2 frames) never shrinks under a long-lived indexer connection. Raw JSONL stays source of truth; a plain re-index RE-APPLIES the cap (restore = raise HEAD/TAIL_CHARS, then re-index). Still open from the parent finding: payload_json ~1.8GB (tool_result 926MB / error 394MB / reasoning 202MB) — needs a per-kind consumer audit before nulling; compact.py precedent covers status_update only.
- **[x] payload_json lever SHIPPED (2026-07-14, cb5c277; closes the last open half of the size-floor finding):** consumer audit found zero readers of events.payload_json for any kind (agent-infra + skills + intel + genomics + evals; only index.py writes, compact.py nulls). Text-backed envelope kinds (tool_result/error/reasoning/tool_call) now dropped at ingest via `trim_payload(…, text)` and backfilled via the extended `agentlogs compact` — kind set defined ONCE in index.py, loaded by compact, equivalence pinned by test. Applied live: 499,167 rows / 1,549MB cleared, DB 6,956→5,630MB, WAL 0B. Sole-copy guard held (2,314 text-empty tool_results kept payload); token_usage (99,540 rows) kept as future usage-analytics source. Size-floor finding now fully closed: DB 10.5GB → 5.6GB same-day at unchanged 30d retention; both fixes are ingest-side so the floor stays down.
