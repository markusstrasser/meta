...
- **[ ] Codex loads stale `~/.codex/skills/genomics-pipeline`:** 6/8 Codex genomics sessions vs repo `.claude/skills/`. **Fix:** dispatch template → repo skills only; parity-sync audit. **Root cause:** skill-router. **Stream:** skill (genomics).
...
- [ ] **Producer-side field contract (the bigger lever — both reviewers, ADR-candidate)** — pulse is the smoke detector; it catches a dead instrument AFTER the fact. The deeper fix: the metric PRODUCER asserts at WRITE time that the field it reads exists + is non-degenerate (e.g. supervision-kpi failing loud if `hook_success` events are absent, instead of silently emitting 0). Catches the next dying field UPSTREAM of the canary. Generalizes the AIR-1591 + the `api.py:193` faked-zeros class. Worth an ADR.
...
- [ ] **Residual (hand to genomics M2 owner): route the MCP STATUS TOOLS through the M2 cache** — M2 repoints `execution_plan`+`sample_remediation`, but `pipeline_status`/`sample_state` in `genomics_mcp.py` still call `view_sample_completion` LIVE = the 1490-calls/timeout agent-facing surface. Confirm M2 (or a follow-up slice) covers the status tools, not just the orchestrator internals.
...
- [ ] **Upgrade `stop-stance-flip-shadow.sh` from lexical markers to a Haiku semantic predicate** — Claude Code supports `type: prompt` hooks (Haiku, ~$0.001) but ONLY on tool events (PreToolUse/PostToolUse/PermissionRequest); Stop hooks stay `command`-type, so the upgrade is a Haiku call INSIDE the command hook (precedent: `source-check-haiku.py`). Design constraint: the hook is a fresh shadow instrument from another 2026-06-12 session — do NOT swap the predicate mid-measurement; ADD `haiku_hit` as a second logged column beside the lexical one so the shadow window yields a lexical-vs-semantic comparison for free, then promote whichever wins. Owner: whoever next touches the stance-flip instrument.
...
- [ ] **skill-usage-watch collects nothing — diagnose**: [~] retired 2026-06-14 — launchd killed; see review-dispatch decision.
...
- [ ] CC 2.1.186 `Agent(type)` permission rules now ENFORCED for named subagent spawns — audit `settings.json` (global + project) for any `Agent(...)` allow/deny rule that now silently blocks an intended dispatch lane (we dispatch named subagents heavily). Source: research/trending-scout-2026-06-24.md #1.
- [ ] CC 2.1.187 `sandbox.credentials` — enable to block sandboxed commands from reading credential files + secret env vars in autonomous/sandbox-mode runs (cheap security harden). Source: trending-scout-2026-06-24 #2.
- [ ] MCPProxy `retrieve_tools` lazy tool-loading — probe the on-demand-tool-search pattern against our always-loaded `.mcp.json` context budget (every subagent spawn pays ~16.7K always-loaded tok); measure context saved before deciding. Source: trending-scout-2026-06-24 #3, extract-pattern.
- [ ] Codex rust-v0.142.0 subagent terminal-error propagation (#28375) — re-check whether the subagent-zero-output gate (claude-code#47936) needs adjustment now that Codex parents RECEIVE terminal subagent errors instead of empty successful completions. Source: trending-scout-2026-06-24 #5, evaluate.
...
- [ ] [2026-06-24] HARNESS-EVAL FLAKY STEP: `system_inventory.py --check` reds non-deterministically (surfaced behind the gov-report/reflect-eval orphan fix this tick — `set -e` had masked it under the orient-drift red). The freshness check renders `architecture.mmd` from the LIVE `launchctl list` loaded-set and compares to the committed snapshot; a transient/one-shot job firing or unloading during the ~40s harness-eval run mutates the set between `orient --drift` (early step) and `system_inventory --check` (late step) → exit 1. Reproduced: red in-gate at 21:46 (render≠committed), green standalone at 21:47 (render=committed), `git diff architecture.mmd` empty both times → no real drift, pure transient set-membership churn. Systematic in-gate (2/2 harness-eval runs red at this step) yet passes standalone — exactly the failure that erodes a gate whose entire value is determinism (constitution P1: a flaky gate gets ignored). **Fix direction:** base the freshness comparison on the STABLE @system-tagged repo plists (`ops/launchd/*.plist`), not the live loaded-set — or exclude one-shot/RunAtLoad-once jobs from the rendered set. CAUTION: `orient --drift` shares the same `render_architecture_mmd()` path, so the fix must preserve real drift detection (verify both green after). Agent-infra-local, checkable (harness-eval determinism across N back-to-back runs). NOT fixed inline — needs design judgment on the stable basis + one-task-per-tick (this tick's pick was the orphan removal, committed ba6eed2). Evidence: `scripts/system_inventory.py:362-366,394-397`; architecture.mmd `L["watch: …loaded-set…"]` line.
