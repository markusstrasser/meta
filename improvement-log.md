...
- [ ] **Complete the usage telemetry (subscription/CLI calls bypass `log_usage`)** — the usage log is 100% `api` transport; every subscription/CLI call (Claude-sub, codex-sub — our DEFAULT routing) is invisible. So the system is blind to its largest usage class and cannot answer "what do we actually spend / how subsidized is the CLI vs API" (operator's 20-40× hunch is unmeasurable). DEEP fix: route all transports through `log_usage`, estimate tokens (tiktoken) where the CLI doesn't expose them. Unlocks real cost/subsidy visibility AND **cost-aware model routing** (a future agent picks by actual $). Foundation under both operator ideas (inline per-call cost + subsidy ratio). Repo: `~/Projects/llmx`. Strictly-better; iatrogenic risk low (estimates labeled).
- [ ] **Generalize `pulse` into the universal instrument registry (deepest RSI leverage)** — today's AIR-1591 bug was ONE instance of "a trusted metric silently goes constant/null and nothing notices." doctor checks, freshness, gov-report, every KPI share the failure mode. `pulse` watches only closure instruments now; make EVERY gating/trusted metric register `{name, producer, last, expected_variance}` so the canary validates the whole set. Then the null-instrument failure class is structurally impossible system-wide — "the system can't lie to itself about its own measurements." Report-only (low iatrogenic). Extends `scripts/pulse.py`.
