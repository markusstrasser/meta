...
- **Trigger:** Markus surfaced context bloat across repos ("genomics for example", "lots of bad context running around", `#f` on MCP scoping). VOI-sequenced (his principle, saved [[feedback-sequence-for-information-gain]]): scout all repos → fan out 4 read-only audits → vet → apply the safe high-VOI cuts → wire a standing canary.
...
### [2026-06-14] [obs] OBSERVE-LOOP all-modes run — both high-volume signals already self-healed same session
