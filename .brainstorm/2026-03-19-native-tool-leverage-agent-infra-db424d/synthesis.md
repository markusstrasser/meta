...
| 17 | 5 | **Spotlight/mdfind for cross-repo search** — `mdfind -onlyin ~/Projects "keyword"` as lightweight alternative to agent-infra MCP for simple queries. | OS is already indexing everything. We built an MCP server when mdfind exists. | Low | initial-claude |
