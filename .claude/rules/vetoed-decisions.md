...
- **Bayesian-surprise fitness for idea selection** (score an idea by posterior shift in a cheap predictor) — 2026-07-02; NO-BUILD-now, PARK with trigger. RSI-ranker consumer is category-mismatched (ranks broken tools, not ideas) + already covered by the decay-salience veto; brainstorm-selection consumer fails the pain-point gate (0 incidents) and the buildable-cheap form is emb-distance (the 06-19 evolver memo's measured win), NOT a posterior — the EFE-style overbuild the 06-16 memo said not to import. Resurrection: a recorded brainstorm mis-pick vs a higher-novelty parked idea in ≥2 sessions → build a *labeled* emb-distance column only. → `decisions/2026-07-02-bayesian-surprise-fitness-no-build.md`
- **Decay-weighted salience for the RSI/decision graph** (Hebbian/Ebbinghaus/Cepeda, from MemPalace `dynamics.py`) — 2026-06-20; read-only ablation killed it: decisions are write-once (47% touched once ever, only 17/62 on >1 day), so the co-access the model needs doesn't exist; recency-only ties full-dynamics 10/10 on top-10. git captures edits, not reads. The real flat-rank gap (`top_priorities.py`) = one sort key (`days_since_last_touch`), not an engine. Resurrection: only if files start being co-accessed across many days — NOT "retune constants." → `decisions/2026-06-20-dynamics-decay-salience-no-build.md`
