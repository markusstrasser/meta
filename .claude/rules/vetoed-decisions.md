...
- Do NOT use codex-cli for trivial queries — ~37K token overhead per call from 9 MCP servers, no flag to disable. Viable for substantial tasks (audits, reviews, code analysis) where GPT-5.4 adds real value. See `dispatch-research` skill for usage patterns.
