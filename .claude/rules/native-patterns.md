...
- "I need to find who calls function X" → `repo-outline.py xrefs . --for X` (already exists)
- "I need to run something daily" → launchd plist, not a Python scheduler
- "I need to track session costs" → `SELECT * FROM v_daily_cost` (view), not a new script
