...
| Want a full audit — loop until no new confirmed bugs | `debug-until-dry <repo>` | **background**: scout wave loop over a shared memo; reads `docs/audit/<date>-bughunt-memo.md` when dry. Knobs: `--max-waves --workers --scouts-per-wave --scout-backend cursor\|codex\|claude (comma-list mixes) --scout-model --scout-effort --scout-timeout --verifier cursor\|codex\|claude\|opus\|none --verifier-model`. Grok 4.5 niche lens: `--scout-backend cursor --scout-model grok-4.5-xhigh` (not a separate backend). Memo JSON carries per-wave token spend (`waves[]`, `token_totals`) |
| PLAN / design needs a Grok cosigner (opt-in) | `critique <plan.md> --axes standard,grok` | foreground; repo-grounded `cursor-agent` axis — see `decisions/2026-07-09-grok-4.5-transport.md` |
