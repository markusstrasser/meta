...
After fixing a correction or critique finding in-session, run this 4-step check:

1. **Fix the instance** — complete the immediate task first.
2. **Hookability check** — ask: "Could a pre-tool or stop hook have caught this
   CLASS of failure before it happened?" (yes/no + one sentence why)
3. **If yes → build the hook in this session.** Don't defer to "a future
   improvement." The cost is near-zero; the compound benefit starts immediately.
4. **If no → document why not** — one `[obs]` line in `improvement-log.md`
