...
## Review Target
A new Claude Code skill that autonomously improves entire codebases. The pipeline:
1. Dump codebase → structured markdown (~400K tokens for a 143-file genomics project)
2. Send to Gemini 3.1 Pro → JSON array of findings (10 categories, severity, files, fixes, verification)
3. Claude triages: extract → disposition table (APPLY/DEFER/REJECT) → human approves
4. Claude executes: for each APPLY finding, edit → verify → git commit OR git revert
5. Post-fix scaffolding: shared error handler, config module, import validator if patterns warrant
6. Report: before/after metrics, commits, failures, remaining items

Designed by Claude. Reviewing for: logical gaps, missing failure modes, overcomplexity, testability.
...
# THE SKILL: project-upgrade/SKILL.md
...
# KEY CONSTRAINTS
- Personal project (1 developer, agent-assisted)
- Python-primary, some Rust/JS
- Target repos: genomics (42-stage Modal pipeline, 115 scripts), intel (investment research), selve (personal knowledge)
- llmx CLI for model dispatch
- Git workflow: all commits to main, prefix with [feature-name]
- Evidence base: instructions alone = 0% reliable, architecture > documentation
