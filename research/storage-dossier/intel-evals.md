...
3. ~~**intel symlink stragglers**: prices/ (256M) + datasets/ + substacks/ (686M) → 2TBPNY~~ **RETRACTED 2026-07-14.** Intel enforces a data-locality policy (`tools/setup-volumes.sh` `LOCAL_ONLY=(sources prices)`, `tools/data_locality_audit.py`): small structured/claim/text stores stay LOCAL because the SSD mount flaps and silences pipelines. `prices/` was localized 2026-06-10 after the daily Prices step crashed twice (06-07, 06-09) on the mount; `research/` stays local so claim views survive an unmount. The 362 large-binary dirs are already symlinked; every remaining local dir is small structured data that MUST stay local. **Do not migrate — it reintroduces a fixed bug.** Only safe intel lever was untracking the churning logs (done). Instant `theses.prev.duckdb` delete (54M) still valid.
