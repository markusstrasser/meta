...
3. **Structured error semantics (SERF).** Current MCP error response is binary `isError` boolean. SERF proposes machine-readable failure categories enabling deterministic agent self-correction (e.g., `{"error_type": "FILE_NOT_FOUND", "recoverable": true, "suggested_action": "search"}`). **Relevance for us:** HIGH — our MCP servers (`agent_infra_mcp.py`, `repo_tools_mcp.py`) return unstructured error strings. SERF would enable our orchestrator to distinguish recoverable from fatal errors without LLM interpretation. `[SOURCE: arXiv:2603.13417]`
