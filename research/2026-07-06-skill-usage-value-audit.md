...
| dataset-register | MERGE → data-acquisition | one pipeline, two index entries (−159 chars, −1 routing hop) |
...
P3. **Fix the two confirmed routing misses** (§7): wire interface-thinking as the named tool of the wakeup-cadence meta/observe front + add trigger vocab (representation/cockpit/instrument); sync diagnose into `~/.agents/skills` and give it ~30 days before demote-or-fold-into-/debug. Demote verify-before to explicit-only (internalized). Merge dataset-register→data-acquisition. Mount-or-archive goals.
