...
- [ ] **Companion skill auto-loader** — Advisory PreToolUse hook that pattern-matches domain keywords (bio/medical → epistemics, llmx commands → llmx-guide) and reminds agents to load mandatory companion skills. Current "invoke if relevant" instructions fail routinely (epistemics bypassed for biotech research, llmx-guide bypassed for llmx calls). Needs design to avoid false positives. (Source: session-analyst 2026-03-07)
